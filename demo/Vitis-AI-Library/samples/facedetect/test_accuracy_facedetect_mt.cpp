...
static ModelInfo get_model_zise(const std::string& filename ) {
  std::string model_name = "/usr/share/vitis_ai_library/models/" + filename + "/" + filename + ".xmodel";
...
//        std::cout << "model width: " << width  << " model heigth: " << height << std::endl;
...
struct FaceDetectReadImages : public ReadImagesThread {
  ...
  virtual int run() override {
    ...
    while (getline(fs, line)) {
      ...
      while (!queue_->push(FrameInfo{++frame_id_, image_resize, single_name, w, h},
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [&] { return vitis::ai::FaceDetect::create(argv[1]); },
      vitis::ai::FaceDetectAcc::instance(argv[3]),
      2,
