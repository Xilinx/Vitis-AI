...
void runResnet50(vart::RunnerExt* runner, const xir::Subgraph* subgraph, int hw_pp_flag, int en_profile) {
  ...
  unsigned char* imageInputs = new unsigned char[inSize * batchSize];
  ...
  signed char* FCResult = new signed char[batchSize * outSize];
  std::vector<vart::TensorBuffer*> inputsPtr, outputsPtr;
  std::vector<std::shared_ptr<xir::Tensor>> batchTensors;

  
  if(hw_pp_flag) {
	pp_kernel_init(handle,"/usr/lib/dpu.xclbin", "pp_pipeline_accel",0,mean,input_scale);
  }
  ...
  for (unsigned int n = 0; n < images.size(); n += batchSize) {
    ...
    for (unsigned int i = 0; i < runSize; i++) {
	  ...
	  std::string img_path = baseImagePath + images[n + i];
  	  if(!en_profile) 
      	     std::cout << "imgpath " << baseImagePath + images[n + i] << std::endl;
	  Mat image;
	  if(hw_pp_flag) {
		//# Hardware preproces
		preprocess(handle, img_path.c_str(), &inHeight, &inWidth, imageInputs + i * inSize);
	  } else {
		//# Software Preprocess	  
		...
		for (int h = 0; h < inHeight; h++) {
			for (int w = 0; w < inWidth; w++) {
				for (int c = 0; c < 3; c++) {
					...
					imageInputs[i * inSize + h * inWidth * 3 + w * 3 + c] = (signed char) var;
				}
			}
		}
	  }	
    ...
    batchTensors.push_back(std::shared_ptr<xir::Tensor>(xir::Tensor::create(
        ...
        xir::DataType{xir::DataType::XINT, sizeof(int8_t) * 8u})));
...
int main(int argc, char* argv[]) {
  // Check args
  if (argc != 4) {
    cout << "Usage of resnet50 demo: ./resnet50 [model_file] [pp_flag] [en_profile]" << endl;
    return -1;
  }
