...
template <int N_STATE, int PROC_TU, int DEPTH_TU, int TMAT_DEPTH, int UQMAT_DEPTH, bool URAM_EN,bool LUTRAM_EN>
...
//	for (int itr1 = 0; itr1 < 1; itr1++) {
		//load_Uq<N_STATE, PROC_TU, DEPTH_TU, TMAT_DEPTH, UQMAT_DEPTH, URAM_EN, LUTRAM_EN>(T_matrix, Uq_matrix);
...
// clang-format off
        #pragma HLS LOOP_TRIPCOUNT min=N_STATE max=N_STATE
        // clang-format on
...
// clang-format off
	...
	for (ap_uint<10> iteration = 0, out_col_start = 0, out_col_cnt = N_STATE; iteration < GEMM_ITERATION;
		...
		gemm_update<N_STATE, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, ABMAT_DEPTH, UMAT_DEPTH, TMAT_DEPTH, URAM_EN, LUTRAM_EN>(
...
template <int N_STATE, int PROC_TU, int DEPTH_TU, int TMAT_DEPTH, int DPDQ_DEPTH, bool URAM_EN, bool LUTRAM_EN>
...
// clang-format off
	...
	for (ap_int<10> u_col_num = N_STATE - 1; u_col_num >= 0; u_col_num--) {
		load_TjDeltaj_vector<N_STATE, PROC_TU, DEPTH_TU, TMAT_DEPTH, DPDQ_DEPTH, URAM_EN, LUTRAM_EN>(
				T_matrix, Tj_vector, Deltaj_vector, D_vector, u_col_num);

		update_T_matrix<N_STATE, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, TMAT_DEPTH, DPDQ_DEPTH, UMAT_DEPTH, URAM_EN, LUTRAM_EN>(
...
#endif
		AU_compute<N_STATE, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, ABMAT_DEPTH, UMAT_DEPTH, TMAT_DEPTH, URAM_EN, LUTRAM_EN>(
...
LOOP2:
    for (int itr1 = 0; itr1 < 1; itr1++) {
		...
		UD_compute<N_STATE, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, TMAT_DEPTH, UMAT_DEPTH, DPDQ_DEPTH, URAM_EN, LUTRAM_EN>(
...
template <int N_STATE, int M_MEAS, int C_CTRL, int PROC_TU, int PROC_MU, bool URAM_EN, bool EKF_EN, int TYPE, int NPC, bool LUTRAM_EN>
...
// clang-format off
    ...
    if (flag_reg[0])
        initialization<N_STATE, M_MEAS, C_CTRL, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, DEPTH_MU_CTRL, UMAT_DEPTH,
                       HMAT_DEPTH, ABMAT_DEPTH, DPDQ_DEPTH, TMAT_DEPTH, UQMAT_DEPTH, URAM_EN, EKF_EN, TYPE, NPC, LUTRAM_EN>(
            A_mat,
#if KF_C != 0
            B_mat,
#endif
            Uq_mat, Dq_mat, H_mat, X0_mat, U0_mat, D0_mat, R_mat, H_matrix, U_matrix, xu_vector, ry_vector, D_vector,
            AB_matrix, T_matrix, Uq_matrix, flag_reg[7], loadX0UD0_en);

    if (flag_reg[1])
        TimeUpdate<N_STATE, M_MEAS, C_CTRL, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, DEPTH_MU_CTRL, UMAT_DEPTH,
                   ABMAT_DEPTH, DPDQ_DEPTH, TMAT_DEPTH,UQMAT_DEPTH, URAM_EN, EKF_EN, TYPE, NPC, LUTRAM_EN>(
            T_matrix, AB_matrix, xu_vector, U_matrix, D_vector, Uq_matrix,
#if KF_C != 0
            u_mat,
#endif
    ...
    if (flag_reg[2])
        MeasUpdate_wrapper<N_STATE, C_CTRL, M_MEAS, PROC_TU, DEPTH_TU, PROC_MU, DEPTH_MU, UMAT_DEPTH, HMAT_DEPTH,
                           TMAT_DEPTH, UQMAT_DEPTH, URAM_EN, EKF_EN, TYPE, NPC, LUTRAM_EN>(
...
void KalmanFilter(xf::cv::Mat<TYPE, N_STATE, N_STATE, NPC>& A_mat,
                  ...
                  xf::cv::Mat<TYPE, N_STATE, N_STATE, NPC>& Uout_mat,
                  xf::cv::Mat<TYPE, N_STATE, 1, NPC>& Dout_mat,
                  unsigned char flag, bool loadX0UD0_en) {
    ...
    assert(((A_mat.rows == N_STATE | A_mat.rows == 0 ) && (A_mat.cols == N_STATE)) && "A matrix dimension must be N_STATE x N_STATE");
#if KF_C != 0
	assert(((B_mat.rows == N_STATE | B_mat.rows == 0 ) && (B_mat.cols == C_CTRL)) && "B matrix dimension must be N_STATE x C_CTRL");
#endif
	assert(((Uq_mat.rows == N_STATE | Uq_mat.rows == 0 ) && (Uq_mat.cols == N_STATE)) && "Uq matrix dimension must be N_STATE x N_STATE");
	assert(((Dq_mat.rows == N_STATE | Dq_mat.rows == 0 ) && (Dq_mat.cols == 1)) && "Dq matrix dimension must be N_STATE x 1");
	assert(((H_mat.rows == M_MEAS | H_mat.rows == 0 ) && (H_mat.cols == N_STATE)) && "H matrix dimension must be M_MEAS x N_STATE");
	assert(((X0_mat.rows == N_STATE | X0_mat.rows == 0 ) && (X0_mat.cols == 1)) && "X0 matrix dimension must be N_STATE x 1");
	assert(((U0_mat.rows == N_STATE | U0_mat.rows == 0 ) && (U0_mat.cols == N_STATE)) && "U0 matrix dimension must be N_STATE x N_STATE");
	assert(((D0_mat.rows == N_STATE | D0_mat.rows == 0 ) && (D0_mat.cols == 1)) && "D0 matrix dimension must be N_STATE x 1");
	assert(((R_mat.rows == M_MEAS | R_mat.rows == 0 ) && (R_mat.cols == 1)) && "R matrix dimension must be M_MEAS x 1");
#if KF_C != 0
	assert(((u_mat.rows == C_CTRL | u_mat.rows == 0 ) && (u_mat.cols == 1)) && "u matrix dimension must be C_CTRL x 1");
#endif
	assert(((y_mat.rows == M_MEAS | y_mat.rows == 0 ) && (y_mat.cols == 1)) && "y matrix dimension must be M_MEAS x 1");
	assert(((Xout_mat.rows == N_STATE | Xout_mat.rows == 0 ) && (Xout_mat.cols == 1)) && "Xout matrix dimension must be N_STATE x 1");
	assert(((Uout_mat.rows == N_STATE | Uout_mat.rows == 0 ) && (Uout_mat.cols == N_STATE)) &&
			"Uout matrix dimension must be N_STATE x N_STATE");
	assert(((Dout_mat.rows == N_STATE | Dout_mat.rows == 0 ) && (Dout_mat.cols == 1)) && "Dout matrix dimension must be N_STATE x 1");
	assert((TYPE == XF_32FC1) && "TYPE must be XF_32FC1");
	assert((NPC == XF_NPPC1) && "NPC must be XF_NPPC1");

	KalmanFilter_def<N_STATE, M_MEAS, C_CTRL, MTU, MMU, USE_URAM, EKF_EN, TYPE, NPC, USE_LUTRAM>(
			A_mat,
#if KF_C != 0
        B_mat,
#endif
        ...
        u_mat,
#endif
			y_mat, Xout_mat, Uout_mat, Dout_mat, flag, loadX0UD0_en);
}
} // namespace cv
} // namespace xf
