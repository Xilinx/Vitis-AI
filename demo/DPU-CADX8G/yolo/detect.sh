...
if [ -f <path-to-vitis-ai>/setup/alveo/DPU-CADX8G/overlaybins/setup.sh ]; then
...
export PYTHONPATH=$PYTHONPATH:${VAI_ALVEO_ROOT}/DPU-CADX8G/yolo
...
if [[ ( -v NETWORK_HEIGHT ) && ( -v NETWORK_WIDTH ) ]]; then
  ...
  python $VAI_ALVEO_ROOT/DPU-CADX8G/yolo/modify_network_dims.py --input_deploy_file $NET_DEF --output_deploy_file $QUANT_DIR/new_dim.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH
  ...
  python $VAI_ALVEO_ROOT/DPU-CADX8G/yolo/modify_network_dims.py --input_deploy_file $NET_DEF_FPGA --output_deploy_file $QUANT_DIR/new_dim_fpga.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH
  if [[ $? != 0 ]]; then echo "Network modification failed. Exiting ..."; exit 1; fi;
...
if [ $ZELDA -eq "0" ]; then
  ...
  python $VAI_ALVEO_ROOT/DPU-CADX8G/yolo/$TEST $BASEOPT 2>&1 | tee single_img_out.txt
