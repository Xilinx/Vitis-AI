...
struct VehicleTypeResnet18PtOnnxResult {
  struct Score {
...
class VehicleTypeResnet18PtOnnx : public OnnxTask {
 public:
  static std::unique_ptr<VehicleTypeResnet18PtOnnx> create(
      const std::string& model_name) {
    return std::unique_ptr<VehicleTypeResnet18PtOnnx>(
        new VehicleTypeResnet18PtOnnx(model_name));
  }
  virtual ~VehicleTypeResnet18PtOnnx() {}
  VehicleTypeResnet18PtOnnx(const std::string& model_name)
      : OnnxTask(model_name) {}

  VehicleTypeResnet18PtOnnx(const VehicleTypeResnet18PtOnnx&) = delete;

  std::vector<VehicleTypeResnet18PtOnnxResult> run(
  ...
  void preprocess(const std::vector<cv::Mat>& images,
    ...
    for (auto index = 0; index < valid_batch; ++index) {
      ...
      set_input_image_rgb(resize_image,
                      ...
                      std::vector<float>{0.00392156863f, 0.00392156863f, 0.00392156863f}
                      );
    }
  }

  std::vector<VehicleTypeResnet18PtOnnxResult> postprocess(
      Ort::Value& output_tensor, int valid_batch) {
    std::vector<VehicleTypeResnet18PtOnnxResult> results;
    ...
    for (auto index = 0; index < valid_batch; ++index) {
      ...
      // print_topk(tb_top5);
      VehicleTypeResnet18PtOnnxResult r;

      for (const auto& v : tb_top5) {
        r.scores.push_back(
            VehicleTypeResnet18PtOnnxResult::Score{v.first, v.second});
      }
