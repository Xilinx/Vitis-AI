...
#include "./inception_v3_pt_onnx.hpp"
...
namespace vitis {
namespace ai {
struct InceptionV3PtOnnxAccThread : public AccThread {
  InceptionV3PtOnnxAccThread(std::string output_file)
      : AccThread(), of(output_file, std::ofstream::out) {
    dpu_result.frame_id = -1;
  }
  virtual ~InceptionV3PtOnnxAccThread() { of.close(); }
  static std::shared_ptr<InceptionV3PtOnnxAccThread> instance(
      std::string output_file) {
    static std::weak_ptr<InceptionV3PtOnnxAccThread> the_instance;
    std::shared_ptr<InceptionV3PtOnnxAccThread> ret;
    if (the_instance.expired()) {
      ret = std::make_shared<InceptionV3PtOnnxAccThread>(output_file);
      the_instance = ret;
    }
  ...
  virtual int run() override {
    ...
    if (getQueue()->pop(dpu_result, std::chrono::milliseconds(5000))) {
      auto res = (InceptionV3PtOnnxResult*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv, [model] { return InceptionV3PtOnnx::create(model); },
      vitis::ai::InceptionV3PtOnnxAccThread::instance(argv[3]), 2);
}
