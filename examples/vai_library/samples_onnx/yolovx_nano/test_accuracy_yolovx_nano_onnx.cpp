...
struct YolovxnanoAcc : public AccThread {
  YolovxnanoAcc(std::string output_file)
      : AccThread(), of(output_file, std::ofstream::out) {
    dpu_result.frame_id = -1;
  }

  virtual ~YolovxnanoAcc() { of.close(); }

  static std::shared_ptr<YolovxnanoAcc> instance(std::string output_file) {
    static std::weak_ptr<YolovxnanoAcc> the_instance;
    std::shared_ptr<YolovxnanoAcc> ret;
    if (the_instance.expired()) {
      ret = std::make_shared<YolovxnanoAcc>(output_file);
      the_instance = ret;
    }
  ...
  void process_result(DpuResultInfo dpu_result) {
    ...
    auto result = (YolovxnanoOnnxResult*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [&] { return YolovxnanoOnnx::create(model_name, 0.001); },
      vitis::ai::YolovxnanoAcc::instance(argv[3]), 2);
}
