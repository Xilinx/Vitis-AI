...
inline BenchMarkResult thread_main_for_performance(const PointsList *points_list,
  ...
  while (!g_stop) {
    ...
    std::vector<float*> vf(batch);
    std::vector<int> vi(batch);
    for (auto n = 0u; n < batch; n++) {
      // imgs.push_back((*points_list)[ret++]);
      vf[n] = const_cast<float*>((*points_list)[ret].data());
      vi[n] = (*points_list)[ret++].size();
    }
    model->run(vf, vi);
...
int main(int argc, char *argv[]) {
  ...
  return vitis::ai::main_for_performance(argc, argv, [model] {
    { return OnnxPointpillars::create(model); }
  });
}
