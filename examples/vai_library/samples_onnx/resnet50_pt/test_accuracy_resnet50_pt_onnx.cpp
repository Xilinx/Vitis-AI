...
namespace vitis {
namespace ai {
struct Resnet50PtOnnxAccThread : public AccThread {
  Resnet50PtOnnxAccThread(std::string output_file)
      : AccThread(), of(output_file, std::ofstream::out) {
    dpu_result.frame_id = -1;
  }
  virtual ~Resnet50PtOnnxAccThread() { of.close(); }
  static std::shared_ptr<Resnet50PtOnnxAccThread> instance(
      std::string output_file) {
    static std::weak_ptr<Resnet50PtOnnxAccThread> the_instance;
    std::shared_ptr<Resnet50PtOnnxAccThread> ret;
    if (the_instance.expired()) {
      ret = std::make_shared<Resnet50PtOnnxAccThread>(output_file);
      the_instance = ret;
    }
  ...
  virtual int run() override {
    ...
    if (getQueue()->pop(dpu_result, std::chrono::milliseconds(5000))) {
      auto res = (Resnet50PtOnnxResult*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv, [model] { return Resnet50PtOnnx::create(model); },
      vitis::ai::Resnet50PtOnnxAccThread::instance(argv[3]), 2);
}
