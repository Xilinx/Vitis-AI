...
void read_s2lr_from_line(const std::string &line, SweepInfo &sweep_info) {
  auto s = line;
  auto cnt = 0u;
...
void read_s2lt_from_line(const std::string &line, SweepInfo &sweep_info) {
  auto s = line;
  auto cnt = 0u;
...
int main( int argc, char *argv[])
{
  if (argc < 5) {
    ...
    std::cout << "usage:" << argv[0] << " [model_0] [model_1] [anno_file_list] [out_path]" << std::endl;
    exit(0);
  }
  ...
  std::string anno_list_name = argv[3];
  ...
  for (auto n = 0u; n < anno_file_names.size(); n+=batch) {
    ...
    for (auto i = 0u; i < input_num; ++i) {
      ...
      read_inno_file(full_name, batch_points_infos[i], 5, batch_points_infos[i].sweep_infos, anno_list_path); 
    }
    auto batch_ret = pointpillars->run(batch_points_infos);
