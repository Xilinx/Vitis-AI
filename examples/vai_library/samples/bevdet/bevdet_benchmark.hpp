...
std::string g_list_name2 = "bin.list";
...
inline BenchMarkResult thread_main_for_performance(const ImageList* image_list,
  ...
  while (!g_stop) {
    ...
    for (size_t i = 0; i < 6; i++) {
      images.emplace_back((*image_list)[6 * ret + i]);
    }
    std::vector<std::vector<char>> bins;
    for (size_t i = 0; i < 4; i++) {
      bins.emplace_back((*bin_list)[4 * ret + i]);
    }
    ret++;
    model->run(images, bins);
...
inline int main_for_performance(int argc, char* argv[], T factory_method) {
  ...
  image_list->resize_images();

  auto bin_list = std::unique_ptr<BinList>(new BinList(g_list_name2));
  if (bin_list->empty()) {
    LOG(FATAL) << "list of bins are empty ";
  }
  //
  ...
  for (int i = 0; i < g_num_of_threads; ++i) {
    results.emplace_back(
        std::async(std::launch::async, thread_main_for_performance<model_t>,
                   image_list.get(), bin_list.get(), std::move(models[i])));
  }
