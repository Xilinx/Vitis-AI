...
static cv::Mat process_result(cv::Mat &image,
                              const vitis::ai::SoloResult &result,
                              bool is_jpeg) {
  auto cate_labels = result.cate_labels;
  auto cate_scores = result.cate_scores;
  for(auto i = 0u; i < cate_labels.size(); i++) {
    //LOG_IF(INFO, is_jpeg) << cate_labels[i] << " " << cate_scores[i];
  }

  Ndarray<int> vis_inds = cate_scores.filter(score_thr);
  auto seg_masks = result.seg_masks;
  auto seg_label  = seg_masks.slice_dim0(vis_inds, true);
  ...
  CHECK_EQ(image.cols, seg_label.shape[2]);
  CHECK_EQ(image.rows, seg_label.shape[1]);
  ...
  for(auto i = 0u; i < num_mask; i++) {
    ...
    LOG_IF(INFO, is_jpeg) << "object: " << i << " " <<  class_name[cate_labels[i]] << "    " <<  std::setprecision(5) << cate_scores[i]; 
