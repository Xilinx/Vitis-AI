...
int main(int argc, char** argv) {
  ...
  if (argc != 4) {
    cout << "usage :" << argv[0] << " <model_name> <image_list_file>  <result_file>"  << endl;
  }
  g_model_name = string(argv[1]);
  image_list_file = string(argv[2]);
  result_file = string(argv[3]);
  std::ifstream fs(image_list_file);
  std::string line;
