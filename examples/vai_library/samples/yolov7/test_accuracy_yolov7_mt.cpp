...
struct YoloAcc : public AccThread {
  YoloAcc(std::string output_file)
      : AccThread(), of(output_file, std::ofstream::out) {
    dpu_result.frame_id = -1;
  }

  virtual ~YoloAcc() { of.close(); }

  static std::shared_ptr<YoloAcc> instance(std::string output_file) {
    static std::weak_ptr<YoloAcc> the_instance;
    std::shared_ptr<YoloAcc> ret;
    if (the_instance.expired()) {
      ret = std::make_shared<YoloAcc>(output_file);
      the_instance = ret;
    }
  ...
  void process_result(DpuResultInfo dpu_result) {
    ...
    auto result = (YOLOv7Result*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [&] { return vitis::ai::YOLOv7::create(model_name + "_acc"); },
      vitis::ai::YoloAcc::instance(argv[3]), 2);
}
