...
int main( int argc, char *argv[])
{
  ...
  for (auto n = 0u; n < anno_file_names.size(); n+=batch) {
    ...
    for (auto i = 0u; i < input_num; ++i) {
      ...
      read_inno_file(full_name, batch_points_infos[i], 16, batch_points_infos[i].sweep_infos, 16, anno_list_path); 
    }
    auto batch_ret = pointpainting->runPointPillars(batch_points_infos);
