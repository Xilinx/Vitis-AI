...
DEF_ENV_PARAM(DEBUG_YOLOV6_ACC, "0");
...
struct YolovXAcc : public AccThread {
  ...
  void process_result(DpuResultInfo dpu_result) {
    LOG_IF(INFO, ENV_PARAM(DEBUG_YOLOV6_ACC))
        << "process" << dpu_result.single_name;
    auto ccoco_id_map_dict = coco_id_map_dict();
    auto result = (YOLOv6Result*)dpu_result.result_ptr.get();
...
int main(int argc, char* argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [&] { return vitis::ai::YOLOv6::create(model_name + "_acc"); },
