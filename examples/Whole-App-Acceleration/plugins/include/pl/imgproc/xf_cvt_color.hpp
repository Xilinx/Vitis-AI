...
void xfyuv42rgb(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_y,
				xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_u,
				xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_v,		
                 xf::cv::Mat<DST_T, ROWS, COLS, NPC>& dst,
                 unsigned short int height,
                 unsigned short int width) {
    XF_CTUNAME(XF_8UC1, NPC) YCRCB[3 * XF_NPIXPERCYCLE(NPC)];
// clang-format off
    #pragma HLS ARRAY_PARTITION variable=YCRCB complete
// clang-format on

    XF_TNAME(SRC_T, NPC) Y_data, U_data, V_data;
	XF_DTUNAME(DST_T, NPC) YCRCB_p[XF_NPIXPERCYCLE(NPC)];
	 XF_TNAME(DST_T, NPC) YCRCB_packed = 0;
    XF_TNAME(DST_T, NPC) RGB_packed = 0;
    XF_DTUNAME(DST_T, NPC) RGB[XF_NPIXPERCYCLE(NPC)];
    XF_DTUNAME(DST_T, NPC) Y, R, B, G;
	uint16_t width_ncpr = ((width + (NPC - 1)) >> XF_BITSHIFT(NPC));
...
// clang-format on
    columnloop:
        for (ap_uint<13> j = 0; j < (width_ncpr >> XF_BITSHIFT(NPC)); j++) {
// clang-format off
            #pragma HLS pipeline
            #pragma HLS LOOP_TRIPCOUNT min=COLS max=COLS
// clang-format on
        	Y_data = in_y.read((i * (width_ncpr >> XF_BITSHIFT(NPC))) + j);
        	U_data = in_u.read((i * (width_ncpr >> XF_BITSHIFT(NPC))) + j);
        	V_data = in_v.read((i * (width_ncpr >> XF_BITSHIFT(NPC))) + j);

            //YCRCB_packed = src.read((i * (width_ncpr >> XF_BITSHIFT(NPC))) + j);
			for (int p = 0; p < XF_NPIXPERCYCLE(NPC); p++) {
			YCRCB_p[p].range((XF_DTPIXELDEPTH(SRC_T, NPC) - 1), 0) = Y_data.range(p * XF_PIXELWIDTH(SRC_T, NPC) + (XF_PIXELWIDTH(SRC_T, NPC) - 1), p * XF_PIXELWIDTH(SRC_T, NPC));
			YCRCB_p[p].range((XF_DTPIXELDEPTH(SRC_T, NPC) * 2) - 1, XF_DTPIXELDEPTH(SRC_T, NPC)) = U_data.range(p * XF_PIXELWIDTH(SRC_T, NPC) + (XF_PIXELWIDTH(SRC_T, NPC) - 1), p * XF_PIXELWIDTH(SRC_T, NPC));
			YCRCB_p[p].range((XF_DTPIXELDEPTH(SRC_T, NPC) * 3) - 1, XF_DTPIXELDEPTH(SRC_T, NPC) * 2) = V_data.range(p * XF_PIXELWIDTH(SRC_T, NPC) + (XF_PIXELWIDTH(SRC_T, NPC) - 1), p * XF_PIXELWIDTH(SRC_T, NPC));
			
			YCRCB_packed.range(p * XF_PIXELWIDTH(DST_T, NPC) + (XF_PIXELWIDTH(DST_T, NPC) - 1),
                                 p * XF_PIXELWIDTH(DST_T, NPC)) = YCRCB_p[p];
			}
            ...
            dst.write(i * (width_ncpr >> XF_BITSHIFT(NPC)) + j, RGB_packed);
        }
    }
}
...
void yuv42rgb(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_y, xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_u, xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& in_v, xf::cv::Mat<DST_T, ROWS, COLS, NPC>& _dst) {
// clang-format off
    #pragma HLS INLINE OFF
// clang-format on

    xfyuv42rgb<SRC_T, DST_T, ROWS, COLS, NPC, XF_WORDWIDTH(DST_T, NPC), XF_WORDWIDTH(DST_T, NPC),
                (ROWS * (COLS >> (XF_NPIXPERCYCLE(NPC))))>(in_y, in_u, in_v,  _dst, in_y.rows, in_y.cols);
}
