...
void runResnet50(vart::RunnerExt *runner, const xir::Subgraph *subgraph, int sw_pp_flag, int no_zcpy,const string baseImagePath,string label_path,int verbose)
{
	...
	if (!sw_pp_flag)
		preprocessor = pp_kernel_init(height, width, no_zcpy, input_scale, mean);
	...
	for (unsigned int n = 0; n < images.size(); n += batch)
	{
		...
		for (unsigned int i = 0; i < runSize; i++)
		{
			...
			{
				cout<<"The image file "<<baseImagePath + "/" + images[n + i]<<" doesnot exist in the image directory "<<baseImagePath<<endl;  
			    exit(1);
			}

			std::string img_name = baseImagePath + "/" + images[n + i];
            Mat image; 
			...
			if (!sw_pp_flag)	
			{
				//# Hardware preproces
				if(!no_zcpy)					
					preprocess(preprocessor, img_name.c_str(), height, width, dpu_input_phy_addr[i], no_zcpy);
				else
					preprocess(preprocessor, img_name.c_str(), height, width, data_in_addr[i], no_zcpy);	
			...
			else
			{
				...
				{
					cout<<"Image resolution of "<<baseImagePath + "/" + images[n + i]<<" exceeds the maximum permitted (SKIPPING)"<< endl ;
					continue;
				}
	...
	if(!verbose)
	{
		...
		std::cout << "   E2E Performance: " << 1000000.0 / ((float)(e2e_time / total_images)) << " fps\n";
		std::cout << "   Pre-process Latency: " << (float)(pre_time / total_images) / 1000 << " ms\n";
		std::cout << "   Execution Latency: " << (float)(exec_time / total_images) / 1000 << " ms\n";
		std::cout << "   Post-process Latency: " << (float)(post_time / total_images) / 1000 << " ms\n";
	}    
#if EN_BRKP
...
int main(int argc, char *argv[])
{
	if (argc < 6 || argc > 7)
	{
		cout << "Usage of ResNet50: ./resnet50 <model>  <image directory> <use_sw_pre_proc (1 for sw pre / 0 for hw pre)> <no_zero_copy (1 for no zero copy / 1 for zero copy)> <verbose (1 for printing the detection coordinates else 0)> <label_file <path> (OPTIONAL)>" << endl;
		return -1;
	}
