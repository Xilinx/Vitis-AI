...
ifeq ($(DPUver), DPUCZDX8G)
ifeq ($(ADDON), JPEG)
	$(MAKE) -C $(ACCEL_DIR)/$(ACCEL)/pl xo TARGET=hw BOARD=Zynq ARCH=aarch64 BLOB_CHANNEL_SWAP_EN=0 BLOB_CROP_EN=0 BLOB_LETTERBOX_EN=0 BLOB_JPEG_EN=1 BLOB_NPC=1
else
	$(MAKE) -C $(ACCEL_DIR)/$(ACCEL)/pl xo TARGET=hw BOARD=Zynq ARCH=aarch64 BLOB_CHANNEL_SWAP_EN=0 BLOB_CROP_EN=0 BLOB_LETTERBOX_EN=0 BLOB_JPEG_EN=0 BLOB_NPC=1
endif
else ifeq ($(DPUver), DPUCAHX8H_3ENGINE)
	$(MAKE) -C $(ACCEL_DIR)/$(ACCEL)/pl xo TARGET=hw BLOB_CHANNEL_SWAP_EN=0 BLOB_CROP_EN=0 BLOB_LETTERBOX_EN=0 BLOB_JPEG_EN=0 BLOB_NPC=1
endif
