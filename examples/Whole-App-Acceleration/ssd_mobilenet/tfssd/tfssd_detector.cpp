...
template <typename T>
void TFSSDdetector::Detect(TFSSDResult* result) {
...
std::cout << "out ptr invalid...\n";
  ...
  runHWSort(pphandle, sort_score, sort_index, sort_size, loc_data);
...
//  __TIC__(NMS)
  for (unsigned int c = 1; c < num_classes_; ++c) {
    ...
    ApplyOneClassNMS(bboxes,  c, sort_score+TOPK_SORT*c, sort_index+TOPK_SORT*c, sort_size[c], score_index_vec[c], &(indices[c]));
...
template void TFSSDdetector::Detect<int8_t>(TFSSDResult* result);

//template <typename T>
//void TFSSDdetector::ApplyOneClassNMS(
//    const T (*bboxes)[4], int label,
//    int8_t *sort_score, short *sort_index, short sort_size,
...
//  while (i < sort_size) {
//    //__TIC__(Decode)
//    const int idx = sort_index[i];
//    const float score = sort_score[i];
...
//    int8_t *sort_score, short *sort_index, short sort_size, const vector<pair<float, int>>& score_index_vec, vector<pair<float,int>>* indices);
//template void TFSSDdetector::ApplyOneClassNMS(
...
void TFSSDdetector::GetOneClassMaxScoreIndex(
  ...
  for (int i = 0; i < num_priors_; ++i) {
    ...
    //cout << "compare " <<score <<" vs "<< nms_confidence_<< " ";
