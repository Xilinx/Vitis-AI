...
parser.add_argument('--onboard', action='store_true', help="running with onboard or not.")
...
def onboard_decorator(func):
    def wrapper(*_args, **_kwargs):
        # Extract arguments from keyword arguments
        args = _kwargs.get("args")
        img = _kwargs.get("img")
        wego_mod = _kwargs.get("wego_mod")
        if args.onboard:
            result = None

            # Create an OnBoard SummaryCollector object with the specified log directory and model name as a comment
            collector = SummaryCollector(log_dir="OnBoard_info", comment=model_config['model']['name'])
            collector.add_platform_info()
            collector.add_graph(wego_mod, input_to_model=torch.unsqueeze(img, 0), verbose=False)

            # Call the original function and get result to generate a tag for add_image
            result = func(*_args, **_kwargs)

            # Add the image with the specified tag to the OnBoard summary, using HWC data format
            # Here, we use img_url instead of the 'img' tensor because the preprocessing applied 
            # to the 'img' tensor which includes normalization, may cause image distortion.
            if result:
                categories, top5_catid, top5_prob = result
                tag = "{}:{}".format(categories[top5_catid[0]], top5_prob[0].item())
                collector.add_image(tag, args.img_url, dataformats='HWC')

            collector.close()
            print("[OnBoard] Now you can run tensorboard to checkout the information collected by OnBoard.")
            print("[OnBoard] like `tensorboard --logdir OnBoard_info`")

        else:
            # Call the original function without OnBoard
            func(*_args, **_kwargs)
            print("[Info] running without OnBoard, if you want to try our visualization tool, turn on onboard in run.sh")
    return wrapper
...
def main():
    ...
    if args.mode == 'normal':
        ...
        run_normal(args=args, img=img, wego_mod=wego_mod)
    elif args.mode == 'perf':
        print("[Info] running in perf mode...")
        run_perf(args=args, img=img, batch=batch, wego_mod=wego_mod)
    else:
