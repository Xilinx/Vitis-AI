...
if __name__ == "__main__":
  ...
  parser.add_argument("--anno-type", default = "classification",
      help="The type of annotation {classification, detection, seg_detection, lane_seg_detection}.")
  parser.add_argument("--label-type", default = "xml",
      help="The type of label file format for detection {xml, json, txt, txtOri}.")
  ...
  with open(list_file, "r") as lf:
    for line in lf.readlines():
      if anno_type == "classification" or anno_type == "detection":
        img_file, anno = line.strip("\n").split(" ")
      elif anno_type == "seg_detection":
        img_file, anno, seg = line.strip("\n").split(" ")
      elif anno_type == "lane_seg_detection":
        img_file, anno, seg, lane = line.strip("\n").split(" ")
      ...
      if anno_type == "classification":
        ...
        if not os.path.exists(root_dir + anno):
          ...
          sys.exit()
      elif anno_type == "seg_detection" or anno_type == "lane_seg_detection":
        if not os.path.exists(root_dir + anno):
          print("annotation file: {} does not exist".format(root_dir + anno))
          sys.exit()
        if not os.path.exists(root_dir + seg):
          print("segmentation label file: {} does not exits".format(root_dir + seg))
          sys.exit()
      break
  # check if label map file exist
  if anno_type == "detection" or anno_type == "seg_detection":
