...
bool UpgradeV1LayerParameter(const V1LayerParameter& v1_layer_param,
  ...
  for (int i = 0; i < v1_layer_param.blobs_size(); ++i) {
    ...
    // Copying num, channels, height, width to 4D shape.
    const auto& cur_blob = v1_layer_param.blobs(i);
    if (cur_blob.has_num() && cur_blob.has_channels() &&
        cur_blob.has_height() && cur_blob.has_width()) {
      auto* lp_shape = layer_param->mutable_blobs(i)->mutable_shape();
      if (v1_layer_param.type() != V1LayerParameter_LayerType_INNER_PRODUCT) {
        lp_shape->add_dim(cur_blob.num());
        lp_shape->add_dim(cur_blob.channels());
      }
      lp_shape->add_dim(cur_blob.height());
      lp_shape->add_dim(cur_blob.width());
    }
  }
