...
def get_arguments():
    ...
    #data config
    parser.add_argument("--imgpath", type=str, default='./data/cityscapes/leftImg8bit/val/',
...
def main():  
    ...
    for c in categories:
        c_items = [name.split('.png')[0] for name in os.listdir(os.path.join(img_path, c))]
        for it in c_items:
            image_name = it
            count += 1
            print('process image id: {}/{}'.format(count, 500))
            image_file = os.path.join(img_path, c, it + '.png')
            ...
            if args.colorFormat:
                color_to_save.save(os.path.join(save_path, image_name + '_with_color.png'))
