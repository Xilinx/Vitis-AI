...
def convert_lmdb(image_root, convert_file, db_file, height, width):
    ...
    cmd = 'python ' + caffe_root +  'python/create_annoset.py --label-type=%s --anno-type=%s --label-map-file=%s --min-dim=%d --max-dim=%d --resize-width=%d --resize-height=%d --check-label %s %s %s %s ../../data/link_480_360/' %\
...
if __name__ == "__main__":
    ...
    get_image_size_cmd = caffe_root + 'build/tools/get_image_size' + ' ' + './' + ' '  + args.val_image2anno + ' ' + '../../data/test_name_size.txt'
