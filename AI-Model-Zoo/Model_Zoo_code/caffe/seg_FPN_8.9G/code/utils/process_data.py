...
def create_list(root_dir, images_dir, annotations_dir, lists_dir, split, image_suffix, annotation_suffix):
    ...
    if not os.path.exists(os.path.join(root_dir, lists_dir)):
        os.makedirs(os.path.join(root_dir, lists_dir))
    with open(os.path.join(root_dir, lists_dir, split + '_img_seg.txt'), "w") as wr:
        for i in range(len(image_lists)):
            wr.write("{} {}\n".format(image_lists[i], annotation_lists[i]))
    print(" =====> create {} lists finishes!!!!".format(split))
