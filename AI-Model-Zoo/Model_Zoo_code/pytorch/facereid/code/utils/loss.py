...
def hard_example_mining(dist_mat, labels, return_inds=False):
    ...
    temp1 = dist_mat.clone()
    temp1[is_neg]=0
    temp2 = dist_mat.clone()
    temp2[is_pos]=0
    dist_ap, relative_p_inds = torch.max(temp1, 1, keepdim=True)
    dist_an, relative_n_inds = torch.min(temp2, 1, keepdim=True)
...
class CirclePairwiseLoss(object):
    def __init__(self):
        self.m = 0.45
        self.s = 32
...
#        is_pos = is_pos.bool()
        is_pos = is_pos.type(torch.uint8)
        is_neg = targets.expand(N, N).ne(targets.expand(N, N).t())

        total_loss=0
        for i in range(N):
            s_p = sim_mat[i][is_pos[i]]
            s_n = sim_mat[i][is_neg[i]]
