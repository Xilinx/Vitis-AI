...
void PriorBoxes::Create(vector<shared_ptr<vector<float>>> & vec_priors, SSD_TYPE type) {
    ...
    if (type == SSD_TYPE::VEHICLE) {
      ...
      prior_boxes.emplace_back(PriorBoxes{
          640, 360, 80, 45, variances, {18, 30, 50}, {40, 60, 80}, {2}, 0.5, 8, 8});
      prior_boxes.emplace_back(PriorBoxes{
 ...
 */
void SSD::Init(const string& path, const string& input_node, const string& output_loc,
    ...
    ssd_runners = vitis::ai::DpuRunner::create_dpu_runner(ssdPath.c_str());
    ssd_runner = ssd_runners[0].get();
    getTensorShape(ssd_runner, &shapes,1, {output_loc, output_conf});
...
void SSD::Run(Mat & img, MultiDetObjects * results) {
    ...
    if (ssd_runner->get_tensor_format() == vitis::ai::DpuRunner::TensorFormat::NHWC) {
        for (int h =0; h< inHeight;h++)
            for (int w =0; w< inWidth;w++)
                for (int c =0; c< 3; c++)
                    imageInputs[h*inWidth*3+w*3+c] =image2.at<Vec3b>(h,w)[c]- mean[c];
    ...
    inputs.push_back(vitis::ai::CpuFlatTensorBuffer(imageInputs,inputTensors[0].get()));
    outputs.push_back(vitis::ai::CpuFlatTensorBuffer(loc,outputTensors[output_mapping[0]].get()));
    outputs.push_back(vitis::ai::CpuFlatTensorBuffer(conf,outputTensors[output_mapping[1]].get()));
    ...
    for (int i = 0; i < conf_size/num_classes_; i++)
        CPUCalcSoftmax(&conf[i*num_classes_], num_classes_, &softmax_data_[i*num_classes_]);
