...
For Classification & Detection example pre-processor accelerator please refer to [classification-preprocess README](./WAA-TRD/accel/classification-pre/README.md) & [detection-preprocess README](./WAA-TRD/accel/detection-pre/README.md) respectively. Array2xfMat, xfMat2hlsStrm & xf::cv::preProcess are manditory submodules in the Pre-process accelerator. Along as interface of the pp_pipeline_accel function is same, user can add or remove other submodules depend on Deep Neural Network's pre-process requirement.  
