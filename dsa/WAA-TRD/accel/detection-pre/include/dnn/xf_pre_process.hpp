...
#pragma HLS UNROLL
				...
				float out_sp = out;
                ap_uint<32>* out_val = (ap_uint<32>*)&out_sp;

                out_pack.range((j * 32) + (32 - 1) + (i * CPW_T * 32), (j * 32) + (i * CPW_T * 32)) =
                    *out_val;
            }
        }
...
#pragma HLS INLINE OFF
    ...
    const int strm_width_out = CPW_T * NPC_T *32;
...
#pragma HLS DATAFLOW
    ...
    OutBitWidthConvert<CPW_T, NPC_T, 32, OUTPUT_PTR_WIDTH_T, strm_width_out>(dstStrmOut, dstStrm, loop_count);
    Strm2Arr<OUTPUT_PTR_WIDTH_T, T_CHANNELS_T, CPW_T, NPC_T, 32>(dstStrm, out, rows, cols);
}
}
}
#endif
