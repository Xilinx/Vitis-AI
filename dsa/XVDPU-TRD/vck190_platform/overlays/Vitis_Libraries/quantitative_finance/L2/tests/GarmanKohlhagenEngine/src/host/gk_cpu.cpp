...
/**
 * @file gk_model.cpp
 * @brief Full precision calculation of Garman-Kohlhagen price and options
...
///
/// This is a simple implementation of the Garman-Kohlhagen closed-form solution
...
/// @param[in]  r_domestic domestic interest rate (decimal form)
/// @param[in]  r_foreign  foreign interest rate (decimal form)
...
void gk_model(double s,
    ...
    double d1 = (std::log(s / k) + (r_domestic - r_foreign + v * v / 2.0) * t) / (v * std::sqrt(t));
    double d2 = d1 - v * std::sqrt(t);

    double pdf_d1 = (1.0 / std::sqrt(2 * M_PI)) * std::exp(-0.5 * d1 * d1);

    if (call) {
        price = s * phi(d1) * std::exp(-r_foreign * t) - k * phi(d2) * std::exp(-r_domestic * t);
        delta = std::exp(-r_foreign * t) * phi(d1);
        theta = (1.0 / 365) * (-v * s * std::exp(-r_foreign * t) * pdf_d1 / (2 * std::sqrt(t)) +
                               r_foreign * s * std::exp(-r_foreign * t) * phi(d1) -
                               r_domestic * k * std::exp(-r_domestic * t) * phi(d2));
        rho = (1.0 / 100) * k * t * std::exp(-r_domestic * t) * phi(d2);
    } else {
        price = phi(-d2) * k * std::exp(-r_domestic * t) - phi(-d1) * s * std::exp(-r_foreign * t);
        delta = std::exp(-r_foreign * t) * (phi(d1) - 1);
        theta = (1.0 / 365) * (-v * s * std::exp(-r_foreign * t) * pdf_d1 / (2 * std::sqrt(t)) -
                               r_foreign * s * std::exp(-r_foreign * t) * phi(-d1) +
                               r_domestic * k * std::exp(-r_domestic * t) * phi(-d2));
        rho = (-1.0 / 100) * k * t * std::exp(-r_domestic * t) * phi(-d2);
    ...
    gamma = exp(-r_foreign * t) * std::exp(-d1 * d1 / 2) / (s * v * std::sqrt(t) * std::sqrt(2 * M_PI));
    vega = (1.0 / 100) * s * std::exp(-r_foreign * t) * std::sqrt(t) * pdf_d1;
