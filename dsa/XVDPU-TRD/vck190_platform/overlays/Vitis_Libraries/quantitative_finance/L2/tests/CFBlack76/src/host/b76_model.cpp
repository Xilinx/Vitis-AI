...
/**
 * @file b76_model.cpp
 * @brief Full precision calculation of BLACK76 price and options for use in
 * comparisons
 */
...
///
/// This is a simple implementation of the BLACK76 closed-form solution along with
...
/// respectively.
///
/// @param[in]  s     underlying forward price
...
void b76_model(double f,
    ...
    double d1 = (std::log(f / k) + (v * v / 2.0) * t) / (v * std::sqrt(t));
    double d2 = d1 - v * std::sqrt(t);

    double pdf_d1 = (1.0 / std::sqrt(2 * M_PI)) * std::exp(-0.5 * d1 * d1);

    if (call) {
        price = f * phi(d1) * std::exp(-r * t) - k * phi(d2) * std::exp(-r * t);
        delta = std::exp(-r * t) * phi(d1);
        theta = (1.0 / 365) * (-v * f * std::exp(-r * t) * pdf_d1 / (2 * std::sqrt(t)) +
                               r * f * std::exp(-r * t) * phi(d1) - r * k * std::exp(-r * t) * phi(d2));
        rho = (1.0 / 100) * k * t * std::exp(-r * t) * phi(d2);
    } else {
        price = phi(-d2) * k * std::exp(-r * t) - phi(-d1) * f * std::exp(-r * t);
        delta = std::exp(-r * t) * (phi(d1) - 1);
        theta = (1.0 / 365) * (-v * f * std::exp(-r * t) * pdf_d1 / (2 * std::sqrt(t)) -
                               r * f * std::exp(-r * t) * phi(-d1) + r * k * std::exp(-r * t) * phi(-d2));
        rho = (-1.0 / 100) * k * t * std::exp(-r * t) * phi(-d2);
    }

    gamma = exp(-r * t) * std::exp(-d1 * d1 / 2) / (f * v * std::sqrt(t) * std::sqrt(2 * M_PI));
    vega = (1.0 / 100) * f * std::exp(-r * t) * std::sqrt(t) * pdf_d1;
