...
void gk_stream_wrapper(WideStreamType& s_stream,
    ...
    for (unsigned int i = 0; i < size; i += NUM_KERNELS) {
        WideDataType s, v, rd, t, k, rf, price, delta, gamma, vega, theta, rho;
...
#pragma HLS UNROLL
            xf::fintech::cfBSMEngine<DT>(s.data[j], v.data[j], rd.data[j], t.data[j], k.data[j], rf.data[j], call,
...
void gk_kernel(ap_uint<BUS_WIDTH>* s_in,
...
#pragma HLS dataflow
    ...
    gk_stream_wrapper(s_stream, v_stream, rd_stream, t_stream, k_stream, rf_stream, call, num, price_stream,
