...
extern "C" void krnl_gemv(uint32_t p_m,
    ...
    if (done) {
        ...
        ap_wait();
        xf::blas::WideType<uint64_t, BLAS_wideType::t_TypeWidth / 8 / sizeof(uint64_t)> l_t(0);
        uint64_t l_clock = p_clock.read();
        l_t[0] = l_clock;
        p_Apk[p_m / BLAS_parEntries] = l_t;
