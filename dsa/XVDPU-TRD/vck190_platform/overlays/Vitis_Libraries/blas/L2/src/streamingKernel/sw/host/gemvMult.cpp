...
int main(int argc, char** argv) {
    ...
    host_buffer_t<BLAS_dataType> h_r(p_m + BLAS_parEntries);
    ...
    uint64_t clock = *reinterpret_cast<uint64_t*>(&h_r[p_m]);
    cout << "Execution clock cycles is: " << clock << endl;
    cout << "Efficiency is: " << 100.0 * p_m * p_n / BLAS_parEntries / BLAS_numChannels / clock << "%." << endl;
    cout << "HW Execution time is: " << 3e-9 * clock << "s." << endl;
