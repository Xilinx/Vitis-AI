...
*/
#ifndef XF_BLAS_GEMM_CPLUSX_HPP
#define XF_BLAS_GEMM_CPLUSX_HPP
...
void gemmCPlusX(hls::stream<ap_uint<t_MemBits> >& p_xStr,
...
#pragma HLS PIPELINE
        ...
        p_cStr.write(l_instrX[i]);
    }

    uint16_t l_opCode = getOpCode<t_MemWordsPerInstr, t_MemBits>(l_instrX);
    while (l_opCode != OpCodeType::OpControl) {
        MemInstr<t_MemWordsPerInstr * t_MemBits / 8> l_memInstr;

        l_memInstr.template loadMem<t_MemBits / 8>(l_instrX);
        ...
        l_gemmKernel.GemmCPlusX(p_sumStr, p_xStr, p_cStr, l_aColBlocks, l_aRowBlocks, l_bColBlocks, 1);
...
#pragma HLS PIPELINE
        ...
        l_opCode = getOpCode<t_MemWordsPerInstr, t_MemBits>(l_instrX);
    }
}
}
}
