...
class GemmStreamSeq : public Fpga {
   protected:
    cl::Kernel m_Kernels[t_NumKernels];

   public:
    GemmStreamSeq(unsigned int p_deviceId = 0) : Fpga(p_deviceId) {}
    ...
    bool createKernel(unsigned int p_kernelId) {
        ...
        m_Kernels[p_kernelId] = cl::Kernel(m_Program, l_ldName.str().c_str());
    ...
    bool callKernel(unsigned int p_kernelId) {
        ...
        assert(p_kernelId < t_NumKernels);
        cl::Event l_event[2];
        m_CommandQueue.enqueueTask(m_Kernels[p_kernelId], &(m_Mem2FpgaEvents[p_kernelId]), &l_event[0]);
