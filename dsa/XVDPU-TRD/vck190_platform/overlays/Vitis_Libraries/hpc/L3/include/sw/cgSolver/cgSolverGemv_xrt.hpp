...
#include "cgSolverKernel_xrt.hpp"
#include "utils.hpp"
...
class CgSolverGemv {
   public:
    ...
    void solve(int& lastIter, uint64_t& finalClock) {
        lastIter = 0;
        finalClock = 0;
        // XKernel::runAll(m_kernels);
        m_kernelControl.run();
        m_kernelGemv.run();
        m_kernelUpdatePk.run();
        m_kernelUpdateRkJacobi.run();
        m_kernelUpdateXk.run();
    ...
    int verify(const string filePath) {
        ...
        int err = 0;
        for (int i = 0; i < 10; i++) {
            cout << h_x.data()[i] << " " << h_xk.data()[i] << endl;
        }
