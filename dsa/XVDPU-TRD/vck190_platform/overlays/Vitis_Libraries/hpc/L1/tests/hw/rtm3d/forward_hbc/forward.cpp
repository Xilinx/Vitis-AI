...
extern "C" void top(const bool p_sel,
                    ...
                    const DATATYPE p_taperz[MaxB],
                    const DATATYPE p_tapery[MaxB],
                    const DATATYPE p_taperx[MaxB],
                    ...
                    UPB_TYPE p_upb[RTM_x * RTM_y * NTime / nPE]) {
...
#pragma HLS ARRAY_PARTITION variable = l_src complete dim = 1
    ...
    while (cont) {
        ...
        forward<NUM_INST, DATATYPE, IN_TYPE, UPB_TYPE>(p_sel, l_domain, l_s, p_t, l_src, p_v2dt2, p_p0, p_p1, p_po0,
                                                       p_po1, p_pp0, p_pp1, p_ppo0, p_ppo1, p_upb);
        cont = l_domain.next();
    }
}
