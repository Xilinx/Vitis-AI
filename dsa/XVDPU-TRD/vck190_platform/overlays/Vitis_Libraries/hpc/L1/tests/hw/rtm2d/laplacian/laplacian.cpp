...
#pragma HLS DATAFLOW
    hls::stream<t_InType> l_str[NTime + 1];
#pragma HLS ARRAY_PARTITION variable = l_str complete dim = 1
    xf::blas::mem2stream<t_InType>(s[0].getArea(), p_img, l_str[0]);
    for (int i = 0; i < NTime; i++)
#pragma HLS UNROLL
        s[i].laplacian(l_str[i], l_str[i + 1]);
    xf::blas::stream2mem<t_InType>(s[0].getArea(), l_str[NTime], p_out);
}
...
extern "C" void top(const unsigned int p_z,
                    ...
                    const t_InType p_img[HEIGHT * WIDTH / nPE],
                    t_InType p_out[HEIGHT * WIDTH / nPE]) {
    STENCEIL_TYPE l_s[NTime];
