...
#pragma HLS ARRAY_PARTITION variable = l_src complete dim = 1
    ...
    for (int t = 0; t < p_t / RTM_numFSMs; t++) {
        ...
        while (cont) {
            ...
            forward<RTM_numFSMs, RTM_dataType, RTM_type, RTM_upbType>(t & 0x01, l_domain, l_s, t, l_src, p_v2dt2, p_pi0,
                                                                      p_pi1, p_po0, p_po1, p_ppi0, p_ppi1, p_ppo0,
                                                                      p_ppo1, p_upb);
            cont = l_domain.next();
        }
    }
