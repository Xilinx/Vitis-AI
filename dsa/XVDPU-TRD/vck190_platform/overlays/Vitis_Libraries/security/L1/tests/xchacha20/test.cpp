...
#include <iostream>
#endif
void xchacha20Core(hls::stream<ap_uint<256> >& keyStrm,
    ...
    xf::security::xchacha20(keyStrm, nonceStrm, plainStream, ePlainStream, cipherStream, eCipherStream);
}
...
bool test() {
    ...
    hls::stream<ap_uint<192> > nonce1Strm;
    ...
    hls::stream<ap_uint<192> > nonce2Strm;
    ...
    ap_uint<192> nonce = 0;
    for (int i = 0; i < 24; ++i) {
        nonce.range(i * 8 + 7, i * 8) = tsuit.nonce[i];
    }
    nonce1Strm.write(nonce);
    nonce2Strm.write(nonce);
    ...
    xchacha20Core(key1Strm, nonce1Strm, plainTextStrm, ePlainTextStrm, cipherStream, eCipherStream);
...
#endif
    // decrypt cipher text
    xf::security::xchacha20(key2Strm, nonce2Strm, cipher2Strm, eCipher2Strm, deText, eDetext);
