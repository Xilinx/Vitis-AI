...
/**
 * @file bls.hpp
 * @brief Header file for BLS curve pairing functions.
 * This file is part of Vitis Security Library.
 */

#ifndef _XF_SECURITY_BLS_HPP_
#define _XF_SECURITY_BLS_HPP_
...
/**
 * @brief BLS curve pairing. This class template provide basic operators in BLS processing.
 *
 * @tparam W Bit length of curve parameters. Parameters should all be smaller than 2^W.
 */
template <int W>
class bls {
   public:
...
#pragma HLS inline
    ...
    /**
     * @brief perform point addition in BLS curve ( R = P + Q)
     *
    ...
    /**
     * @brief perform point multiply scalar in curve ( R = P * k)
     *
     * @param Px X coordinate of point P. Should be smaller than p.
     * @param Py Y coordinate of point P. Should be smaller than p.
    ...
    /**
     * @brief Setup parameters for curve of y^2 = x^3 + ax + b in GF(p)
     *
    ...
    /**
     * @brief genkey an encrypted message point pair {C1, C2} with privateKey
