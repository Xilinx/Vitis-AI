...
void writeS2M(int n, hls::stream<ap_uint<32> >& inStrm, hls::stream<bool>& endStrm, ap_uint<32>* adler32Result) {
    for (int i = 0; i < n; i++) {
#pragma HLS pipeline ii = 1
        endStrm.read();
        adler32Result[i] = inStrm.read();
    }
    endStrm.read();
}
...
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    32 max_write_burst_length = 2 max_read_burst_length = 16 bundle = gmem1 port = adler
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
...
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 32 num_read_outstanding = \
    1 max_write_burst_length = 16 max_read_burst_length = 2 bundle = gmem3 port = adler32Result
#pragma HLS INTERFACE s_axilite port = num bundle = control
#pragma HLS INTERFACE s_axilite port = size bundle = control
#pragma HLS INTERFACE s_axilite port = len bundle = control
#pragma HLS INTERFACE s_axilite port = adler bundle = control
#pragma HLS INTERFACE s_axilite port = inData bundle = control
#pragma HLS INTERFACE s_axilite port = adler32Result bundle = control
...
#pragma HLS dataflow
    ...
    hls::stream<ap_uint<32> > adler32Strm;
    hls::stream<bool> adler32EndStrm;
...
#pragma HLS stream variable = adlerStrm depth = 16
#pragma HLS stream variable = endStrm depth = 16
#pragma HLS stream variable = adler32Strm depth = 16
#pragma HLS stream variable = adler32EndStrm depth = 16
    readLenM2S(num, len, adler, lenStrm, adlerStrm, endStrm);
    readDataM2S(size, inData, data512Strm);
    splitStrm(size, data512Strm, dataStrm);
    xf::security::adler32<W>(adlerStrm, dataStrm, lenStrm, endStrm, adler32Strm, adler32EndStrm);
    writeS2M(num, adler32Strm, adler32EndStrm, adler32Result);
}
