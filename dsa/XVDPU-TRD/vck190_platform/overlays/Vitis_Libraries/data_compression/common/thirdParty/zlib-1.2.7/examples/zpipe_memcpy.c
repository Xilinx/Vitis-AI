...
int def(FILE* source, FILE* dest, int level) {
    ...
    // read full file into input vector
    fseek(source, 0, SEEK_END);          // seek to end of file
    uint64_t input_size = ftell(source); // get current file pointer
    fseek(source, 0, SEEK_SET);          // seek back to beginning of file
    std::vector<uint8_t> inVec(input_size);
    fread(inVec.data(), 1, input_size, source);

    uint64_t inIdx = 0;
    uint64_t outIdx = 0;

    std::vector<uint8_t> outVec(input_size);
    ...
    uint64_t compressedSize = outIdx;
    ...
    compress_API_time_ms_1 = duration_ms;
    ...
    fwrite(outVec.data(), 1, compressedSize, dest);
