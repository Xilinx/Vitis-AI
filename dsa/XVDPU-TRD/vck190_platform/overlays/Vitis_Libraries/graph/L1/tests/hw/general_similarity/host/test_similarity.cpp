...
int main(int argc, const char* argv[]) {
    ...
    if (graphType == 0) {
        ...
        numVerticesPU[PU_NUMBER - 1] = 10; // vertex number in the last PU
    } else {
    ...
    if (graphType == 0)
        generateSourceParams(numVerticesPU, numEdges, dataType, sourceID, offset32, indice32, weightSparse, sourceNUM,
                             &sourceIndice, &sourceWeight);
    else
        generateSourceParams(numVerticesPU, numEdges, dataType, sourceID, weightDense, sourceNUM, &sourceWeight);

    // calculate similarity
    int info;
    if (graphType == 0)
        info = computeSimilarity(xclbinPath, goldenFile, numVertices, numEdges, similarityType, dataType, sourceID,
                                 sortK, repInt, numVerticesPU, numEdgesPU, offset32, indice32, weightSparse, sourceNUM,
                                 sourceIndice, sourceWeight);
    else
        info = computeSimilarity(xclbinPath, goldenFile, numVertices, numEdges, similarityType, dataType, sourceID,
