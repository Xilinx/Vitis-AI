...
template <int PUNUM>
void readInSource(std::fstream& fstream, // input: file stream
                  int dataType,          // 0:int32, 1:float
                  unsigned int numEdges, // input: edge numbers of each vertex
                  unsigned int batchNUM, // input: batch numbers in source file
    ...
    while (fstream.getline(line, sizeof(line))) {
        ...
        data >> tmp;
        if (dataType == 0) {
            buffer[id] = bitsToFloat<int32_t, float>((int32_t)tmp);
        } else {
            buffer[id] = tmp;
        }
        id++;
    ...
    if (id != numEdges * batchNUM) { // source numbers between file input and sum should match
        std::cout << "Error : sum of source numbers doesn't match file input source number!" << std::endl;
        std::cout << "sum =" << numEdges * batchNUM << " cnt=" << id << std::endl;
        exit(1);
    }
}
...
template <int PUNUM>
void readInDataFile(std::string sourceFile,
    ...
    std::fstream sourcefstream(sourceFile.c_str(), std::ios::in);
    if (!sourcefstream) {
        std::cout << "Error: " << sourceFile << "source file doesn't exist !" << std::endl;
        exit(1);
    }
    ...
    // read in source///////////////////////////////////////////
    *sourceWeight = aligned_alloc<float>(numEdges * batchNUM);
    readInSource<PUNUM>(sourcefstream, dataType, numEdges, batchNUM, sourceWeight[0]);
