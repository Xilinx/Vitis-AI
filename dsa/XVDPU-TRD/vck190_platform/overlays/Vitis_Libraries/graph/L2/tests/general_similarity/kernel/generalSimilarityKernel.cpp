...
#pragma HLS PIPELINE II = 1
        if (graph_type == xf::graph::enums::SPARSE) source_indice.write(indice[i]);
        if (!((similarity_type == xf::graph::enums::JACCARD_SIMILARITY) && (graph_type == xf::graph::enums::SPARSE)))
...
template <int CHNM, int WData>
void loadControl(ap_uint<32> similarity_type,
                 ap_uint<32> graph_type,
...
#pragma HLS INLINE off
    ...
    if (graph_type == xf::graph::enums::SPARSE) {
...
#pragma HLS array_partition variable = data_num complete
    ...
    loadControl<CHNM, WData>(similarity_type, graph_type, vertex_num, edge_num, data_num);
...
#pragma HLS INLINE off
    ...
    for (ap_uint<8> i = 0; i < PU; i++) {
        start_id[i] = config[5 + i];
    }

    for (ap_uint<8> i = 0; i < PU; i++) {
        vertex_num[i] = config[5 + PU + i];
    }

    for (ap_uint<8> i = 0; i < PU; i++) {
        edge_num[i] = config[5 + 2 * PU + i];
    }

    for (ap_uint<8> i = 1; i < 3 * PU + 5; i++) {
        config_strm.write(config[i]);
    }

#ifndef __SYNTHESIS__
    for (int i = 0; i < 3 * PU + 5; i++) std::cout << "config" << i << " = " << config[i] << std::endl;
#endif
}
...
#pragma HLS INLINE
    ...
    loadPU<CHNM, WData>(similarity_type, graph_type, vertex_num[0], edge_num[0], dataIn00, dataIn01, dataIn02, dataIn03,
                        strm0[0], strm1[0], strm2[0], strm3[0]);

    loadPU<CHNM, WData>(similarity_type, graph_type, vertex_num[1], edge_num[1], dataIn10, dataIn11, dataIn12, dataIn13,
                        strm0[1], strm1[1], strm2[1], strm3[1]);

    loadPU<CHNM, WData>(similarity_type, graph_type, vertex_num[2], edge_num[2], dataIn20, dataIn21, dataIn22, dataIn23,
                        strm0[2], strm1[2], strm2[2], strm3[2]);

    loadPU<CHNM, WData>(similarity_type, graph_type, vertex_num[3], edge_num[3], dataIn30, dataIn31, dataIn32, dataIn33,
                        strm0[3], strm1[3], strm2[3], strm3[3]);
}
...
void generalSimilarityTop4PU(ap_uint<32> k,
                             ap_uint<32> source_num,
                             ap_uint<32> similarity_type,
                             ap_uint<32> data_type,
                             ap_uint<32> graph_type,
...
#pragma HLS resource variable = strm_in3 core = FIFO_BRAM
    ...
    loadData4PU<CHNM, WData>(similarity_type, graph_type, vertex_num, edge_num, dataIn00, dataIn01, dataIn02, dataIn03,
                             dataIn10, dataIn11, dataIn12, dataIn13, dataIn20, dataIn21, dataIn22, dataIn23, dataIn30,
                             dataIn31, dataIn32, dataIn33, strm_in0, strm_in1, strm_in2, strm_in3);
...
#pragma HLS resource variable = end_strm0 core = FIFO_SRL
    ...
    xf::graph::generalSimilarity<CHNM, PU, WData, RAM_SZ, true>(config_strm, sourceIndice, sourceWeight, strm_in0,
                                                                strm_in1, strm_in2, strm_in3, row_strm0,
                                                                similarity_strm0, end_strm0);
...
extern "C" void generalSimilarityKernel(ap_uint<32>* config,
...
#pragma HLS INTERFACE m_axi offset = slave latency = 32 num_write_outstanding = 1 num_read_outstanding = \
    32 max_write_burst_length = 8 max_read_burst_length = 8 bundle = gmem4_0 port = sourceIndice depth = 65536
...
#pragma HLS resource variable = config_strm core = FIFO_BRAM
    ...
    loadConfig<PU>(config, k, source_num, similarity_type, data_type, graph_type, start_id, vertex_num, edge_num,
                   config_strm);
...
#pragma HLS resource variable = source_weight core = FIFO_BRAM
    ...
    loadSource<CHANNEL_NUMBER, W_DATA>(similarity_type, graph_type, source_num, sourceIndice, sourceWeight,
                                       source_indice, source_weight);
...
#pragma HLS resource variable = end_strm core = FIFO_SRL
    ...
    generalSimilarityTop4PU<CHANNEL_NUMBER, W_DATA, RAM_SIZE, 64>(
        k, source_num, similarity_type, data_type, graph_type, start_id, vertex_num, edge_num, config_strm,
        source_indice, source_weight,
