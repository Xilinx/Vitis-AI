...
void runResnet50(vart::RunnerExt *runner, const xir::Subgraph *subgraph, int hw_pp_flag, int en_profile, const string baseImagePath)
{
	...
	if (hw_pp_flag)
		pp_kernel_init(pphandle, "/media/sd-mmcblk0p1/dpu.xclbin", mean, input_scale, height, width);
...
int main(int argc, char *argv[])
{
	...
	if (argc != 5)
	{
		cout << "Usage of resnet50: ./bin/resnet50.exe <model> <image directory> <use_hw_pre_process (0 for sw pre / 1 for hw pre)> <verbose (0 for printing the class else 1)>" << endl;
		return -1;
	}
