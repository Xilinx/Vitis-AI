...
int main(const int argc, const char **argv)
{
  if (argc != 3)
  {
    cout << "Usage of ADAS detection: ./bin/adas_detection.exe <model> <use_hw_pre_process (0 for sw pre / 1 for hw pre)> " << endl;
    return -1;
  }
  ...
  */
  ...
  if (usePPFlag)
    pp_kernel_init(pp_handle, "/media/sd-mmcblk0p1/dpu.xclbin", dpu_input_phy_addr, mean, input_scale[0]);
