...
{  
  ...
  /// Load common kernels
  sysMan->loadKernels("../../../../tools/AKS/kernel_zoo");

  /// Load tvm kernels
  sysMan->loadKernels("kernel_zoo");

  /// Load graph
  sysMan->loadGraphs("graph_zoo/graph_resnet50_dpucadf8h_tvm.json");
  ...
  std::cout << "[INFO] Starting enqueue ... " << std::endl;
    //int bt = 3; // Batch Size for U50
    int bt = 4; // Batch Size for U50
  for (int i = 0; i < images.size(); i+=bt) {
    ...
    //sysMan->enqueueJob (graph, batch, std::move(v), nullptr);
