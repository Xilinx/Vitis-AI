...
void WriteProtoToBinaryFile(const Message& proto, const char* filename, int format) {
  fstream output(filename, ios::out | ios::trunc | ios::binary);
  if (typeid(proto) == typeid(BlobProto)) {
    BlobProto blob_proto;
    blob_proto.CopyFrom(proto);
    blob_proto.set_format(format);
    CHECK(blob_proto.SerializeToOstream(&output));
  } else if (typeid(proto) == typeid(NetParameter)) {
    NetParameter net_param;
    net_param.CopyFrom(proto);

    for (int i = 0; i < net_param.layer_size(); ++i) {
      LayerParameter* layer_param = net_param.mutable_layer(i);
      for (int j = 0; j < layer_param->blobs_size(); ++j) {
        layer_param->mutable_blobs(j)->set_format(format);
      }
    }
    ...
    case AnnotatedDatum_AnnotationType_BBOX:
      ...
      if (labeltype == "xml") {
        ...
        return ReadTxtToAnnotatedDatum(labelfile, ori_height, ori_width,
 					name_to_label ,anno_datum);
      } else if (labeltype == "txtOri") {
        return ReadTxtOriToAnnotatedDatum(labelfile, ori_height, ori_width,
 					name_to_label ,anno_datum);
      } else {
...
bool ReadLaneBoxToAnnotatedDatum(const string& filename, const string& lanefile,
				 AnnotatedDatum* anno_datum) {
  int height, width;
  GetImageSize(filename, &height, &width);

  std::ifstream infile(lanefile.c_str());
  if (!infile.good()) {
    LOG(INFO) << "Cannot open " << lanefile;
    return false;
  }
  anno_datum->clear_lane_annotation_group();
  int num;
  int label;
  float xmin, ymin, xmax, ymax;
  infile >> num;
  for (int i = 0; i < num; i++) {
    infile >> xmin >> ymin >> xmax >> ymax >> label;
    ...
    for (int g = 0; g < anno_datum->lane_annotation_group_size(); ++g) {
      AnnotationGroup* anno_group = anno_datum->mutable_lane_annotation_group(g);
    ...
    if (!found_group) {
      ...
      AnnotationGroup* anno_group = anno_datum->add_lane_annotation_group();
      anno_group->set_group_label(label);
      anno = anno_group->add_annotation();
      instance_id = 0;
    }
    anno->set_instance_id(instance_id++); // FIXME: now the grid file do not contain the instance-id information
    LOG_IF(WARNING, xmin > width) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, ymin > height) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, xmax > width) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, ymax > height) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, xmin < 0) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, ymin < 0) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, xmax < 0) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, ymax < 0) << lanefile <<
      " bounding box exceeds image boundary.";
    LOG_IF(WARNING, xmin > xmax) << lanefile <<
      " bounding box irregular.";
    LOG_IF(WARNING, ymin > ymax) << lanefile <<
      " bounding box irregular.";
...
bool ReadSegImageToAnnotatedDatum(const string& filename,
    const string& labelfile, const string& segname, const int height, const int width,
    const int min_dim, const int max_dim, const bool is_color,
    const string& encoding, const AnnotatedDatum_AnnotationType type,
    const string& labeltype, const std::map<string, int>& name_to_label,
    AnnotatedDatum* anno_datum) {
  ...
  bool status_seg = ReadImageToDatum(segname, -1, height, width,
                                 min_dim, max_dim, false, encoding,
                                 anno_datum->mutable_seg_datum());
...
// Parse plain txt detection annotation: label_id, xmin, ymin, xmax, ymax, angle.
bool ReadTxtOriToAnnotatedDatum(const string& labelfile, const int height,
    const int width, const std::map<string ,int>&  name_to_label ,AnnotatedDatum* anno_datum) {
  std::ifstream infile(labelfile.c_str());
  if (!infile.good()) {
    LOG(INFO) << "Cannot open " << labelfile;
  ...
  string label_tmp,image_name;
  float xmin, ymin, xmax, ymax , angle;
  while (infile >> image_name >> label_tmp >> xmin >> ymin >> xmax >> ymax >> angle) {
    if(xmin == xmax || ymin == ymax)
      continue;
    xmin = std::min(xmin,xmax);
    ymin = std::min(ymin,ymax);
    ...
    bbox->set_xmax(static_cast<float>(xmax) / width);
    bbox->set_ymax(static_cast<float>(ymax) / height);
    bbox->set_angle_cos(static_cast<float>(std::cos(angle/180.0 * pi)));
    bbox->set_angle_sin(static_cast<float>(std::sin(angle/180.0 * pi)));
    bbox->set_difficult(false);
  }
  return true;
}
