...
void ClipBBox(const NormalizedBBox& bbox, const float height, const float width,
  ...
  if(bbox.label_with_ori()){
    clip_bbox->set_angle_cos(bbox.angle_cos());
    clip_bbox->set_angle_sin(bbox.angle_sin());
    }
  clip_bbox->clear_size();
  clip_bbox->set_size(BBoxSize(*clip_bbox));
  clip_bbox->set_difficult(bbox.difficult());
...
void ScaleBBox(const NormalizedBBox& bbox, const int height, const int width,
  ...
  if(bbox.label_with_ori()){
    scale_bbox->set_angle_cos(bbox.angle_cos());
    scale_bbox->set_angle_sin(bbox.angle_sin());
    }
...
void LocateBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  if(bbox.label_with_ori()){
    loc_bbox->set_angle_cos(bbox.angle_cos());
    loc_bbox->set_angle_sin(bbox.angle_sin());
    }
...
bool ProjectBBox(const NormalizedBBox& src_bbox, const NormalizedBBox& bbox,
  ...
  if(bbox.label_with_ori()){
    proj_bbox->set_angle_cos(bbox.angle_cos());
    proj_bbox->set_angle_sin(bbox.angle_sin());
    }
...
void EncodeBBox(
  ...
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    if (encode_variance_in_target) {
      ...
      if(bbox.label_with_ori()){
        encode_bbox->set_angle_cos(bbox.angle_cos());
        encode_bbox->set_angle_sin(bbox.angle_sin());
       }
    } else {
...
void DecodeBBox(
  ...
  if (code_type == PriorBoxParameter_CodeType_CORNER) {
    if (variance_encoded_in_target) {
      ...
      if(bbox.label_with_ori()){
         decode_bbox->set_angle_cos(bbox.angle_cos());
         decode_bbox->set_angle_sin(bbox.angle_sin());
         }
    } else {
    ...
    float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
    CHECK_GE(prior_width, 0);
    float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
    CHECK_GE(prior_height, 0);
    float prior_center_x = (prior_bbox.xmin() + prior_bbox.xmax()) / 2.;
    float prior_center_y = (prior_bbox.ymin() + prior_bbox.ymax()) / 2.;
...
void MineHardExamples(const Blob<Dtype>& conf_blob,
  ...
  *num_negs = 0;
  ...
  bool ori_flag = multibox_loss_param.ori_flag();
  ...
  if (mining_type == MultiBoxLossParameter_MiningType_HARD_EXAMPLE) {
    ...
    if (*num_matches != 0) {
      ...
      if(ori_flag) loc_shape[1] = *num_matches * 6;
      else loc_shape[1] = *num_matches * 4;
    ...
    if(ori_flag)
       ComputeLocOriLoss(loc_pred, loc_gt, *all_match_indices, num,
...
void GetGroundTruth(const Dtype* gt_data, const int num_gt,
  ...
  for (int i = 0; i < num_gt; ++i) {
    NormalizedBBox bbox;
    int start_idx;
    if(bbox.label_with_ori()) start_idx = i * 10;
    else start_idx = i * 8;
    ...
    if(bbox.label_with_ori()){
       bbox.set_angle_cos(gt_data[start_idx + 7]);
       bbox.set_angle_sin(gt_data[start_idx + 8]);
       }
...
template <typename Dtype>
void GetLocOriPredictions(const Dtype* loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds) {
  loc_preds->clear();
  if (share_location) {
  ...
  for (int i = 0; i < num; ++i) {
    LabelBBox& label_bbox = (*loc_preds)[i];
    for (int p = 0; p < num_preds_per_class; ++p) {
      int start_idx = p * num_loc_classes * 6;
      for (int c = 0; c < num_loc_classes; ++c) {
        int label = share_location ? -1 : c;
        if (label_bbox.find(label) == label_bbox.end()) {
          label_bbox[label].resize(num_preds_per_class);
        }
        label_bbox[label][p].set_xmin(loc_data[start_idx + c * 6]);
        label_bbox[label][p].set_ymin(loc_data[start_idx + c * 6 + 1]);
        label_bbox[label][p].set_xmax(loc_data[start_idx + c * 6 + 2]);
        label_bbox[label][p].set_ymax(loc_data[start_idx + c * 6 + 3]);
        label_bbox[label][p].set_angle_cos(loc_data[start_idx + c * 6 + 4]);
        label_bbox[label][p].set_angle_sin(loc_data[start_idx + c * 6 + 5]);
      }
    }
    loc_data += num_preds_per_class * num_loc_classes * 6;
  }
}
...
template void GetLocOriPredictions(const float* loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds);
template void GetLocOriPredictions(const double* loc_data, const int num,
      const int num_preds_per_class, const int num_loc_classes,
      const bool share_location, vector<LabelBBox>* loc_preds);
...
void EncodeLocPrediction(const vector<LabelBBox>& all_loc_preds,
  ...
  const bool use_prior_for_matching =
      multibox_loss_param.use_prior_for_matching();
  const bool ori_flag = multibox_loss_param.ori_flag();
  int count = 0;
  for (int i = 0; i < num; ++i) {
    for (map<int, vector<int> >::const_iterator
      ...
      for (int j = 0; j < match_index.size(); ++j) {
        ...
        if(gt_encode.label_with_ori()){
           loc_gt_data[count * 6] = gt_encode.xmin();
           loc_gt_data[count * 6 + 1] = gt_encode.ymin();
           loc_gt_data[count * 6 + 2] = gt_encode.xmax();
           loc_gt_data[count * 6 + 3] = gt_encode.ymax();
           loc_gt_data[count * 6 + 4] = gt_encode.angle_cos();
           loc_gt_data[count * 6 + 5] = gt_encode.angle_sin();
           }
        else{
        ...
        if (bp_inside) {
          ...
          if(ori_flag){
            loc_pred_data[count * 6] =
                (match_bbox.xmin() < 0 || match_bbox.xmin() > 1) ?
                gt_encode.xmin() : loc_pred[j].xmin();
            loc_pred_data[count * 6 + 1] =
                (match_bbox.ymin() < 0 || match_bbox.ymin() > 1) ?
                gt_encode.ymin() : loc_pred[j].ymin();
            loc_pred_data[count * 6 + 2] =
                (match_bbox.xmax() < 0 || match_bbox.xmax() > 1) ?
                gt_encode.xmax() : loc_pred[j].xmax();
            loc_pred_data[count * 6 + 3] =
                (match_bbox.ymax() < 0 || match_bbox.ymax() > 1) ?
                gt_encode.ymax() : loc_pred[j].ymax();
            loc_pred_data[count * 6 + 4] =
                (match_bbox.angle_cos() < 0 || match_bbox.angle_cos() > 1) ?
                gt_encode.angle_cos() : loc_pred[j].angle_cos();
            loc_pred_data[count * 6 + 5] =
                (match_bbox.angle_sin() < 0 || match_bbox.angle_sin() > 1) ?
                gt_encode.angle_sin() : loc_pred[j].angle_sin();
             } 
          else{
          ...
          if(ori_flag){
            loc_pred_data[count * 6] = loc_pred[j].xmin();
            loc_pred_data[count * 6 + 1] = loc_pred[j].ymin();
            loc_pred_data[count * 6 + 2] = loc_pred[j].xmax();
            loc_pred_data[count * 6 + 3] = loc_pred[j].ymax();
            loc_pred_data[count * 6 + 4] = loc_pred[j].angle_cos();
            loc_pred_data[count * 6 + 5] = loc_pred[j].angle_sin();
              }
          else{
...
template <typename Dtype>
void ComputeLocOriLoss(const Blob<Dtype>& loc_pred, const Blob<Dtype>& loc_gt,
      const vector<map<int, vector<int> > >& all_match_indices,
      const int num, const int num_priors, const LocLossType loc_loss_type,
      vector<vector<float> >* all_loc_loss) {
  int loc_count = loc_pred.count();
...
// Explicit initialization.
template void ComputeLocOriLoss(const Blob<float>& loc_pred,
      const Blob<float>& loc_gt,
      const vector<map<int, vector<int> > >& all_match_indices,
      const int num, const int num_priors, const LocLossType loc_loss_type,
      vector<vector<float> >* all_loc_loss);
template void ComputeLocOriLoss(const Blob<double>& loc_pred,
      const Blob<double>& loc_gt,
      const vector<map<int, vector<int> > >& all_match_indices,
      const int num, const int num_priors, const LocLossType loc_loss_type,
      vector<vector<float> >* all_loc_loss);
...
void GetDetectionResults(const Dtype* det_data, const int num_det,
  ...
  for (int i = 0; i < num_det; ++i) {
    NormalizedBBox bbox;
    int start_idx;
    if(bbox.label_with_ori()) start_idx = i * 9;
    else start_idx = i * 7;
    ...
    if(bbox.label_with_ori()){
      bbox.set_angle_cos(det_data[start_idx + 7]);
      bbox.set_angle_sin(det_data[start_idx + 8]);
      }
    float bbox_size = BBoxSize(bbox);
    bbox.set_size(bbox_size);
...
template <typename Dtype>
void ApplyNMSOriFast(const Dtype* bboxes, const Dtype* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, vector<int>* indices) {
  // Get top_k scores (with corresponding indices).
  vector<pair<Dtype, int> > score_index_vec;
  ...
  while (score_index_vec.size() != 0) {
    ...
    for (int k = 0; k < indices->size(); ++k) {
      if (keep) {
        const int kept_idx = (*indices)[k];
        float overlap = JaccardOverlap(bboxes + idx * 6, bboxes + kept_idx * 6);
        keep = overlap <= adaptive_threshold;
      } else {
        break;
      }
...
template
void ApplyNMSOriFast(const float* bboxes, const float* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, vector<int>* indices);
template
void ApplyNMSOriFast(const double* bboxes, const double* scores, const int num,
      const float score_threshold, const float nms_threshold,
      const float eta, const int top_k, vector<int>* indices);
