...
Blob<Dtype>::Blob(const int num, const int channels, const int height,
    const int width, bool enable_mask)
  // capacity_ must be initialized before calling Reshape
  : capacity_(0), enable_mask_(enable_mask) {
  Reshape(num, channels, height, width);
}

template <typename Dtype>
Blob<Dtype>::Blob(const vector<int>& shape, bool enable_mask)
  // capacity_ must be initialized before calling Reshape
  : capacity_(0), enable_mask_(enable_mask) {
  Reshape(shape);
}
...
template <> void Blob<bool>::Update(int compression) {
  NOT_IMPLEMENTED;
}
template <> void Blob<unsigned int>::Update(int compression) {
  NOT_IMPLEMENTED;
}
template <> void Blob<int>::Update(int compression) {
  NOT_IMPLEMENTED;
}

template <typename Dtype>
void Blob<Dtype>::Update(int compression) {
  ...
  case Caffe::GPU:
    if (copy_diff) {
      ...
      caffe_copy(count_, source.gpu_data(),
          static_cast<Dtype*>(data_->mutable_gpu_data()));
      if (source.enable_mask() && enable_mask_){
        caffe_copy(count_, source.gpu_mask(),
            static_cast<Dtype*>(mask_->mutable_gpu_data()));
      } else if (source.enable_mask() && !enable_mask_) {
        LOG(INFO) << "CopyFrom: Ignore mask in source blob, because enable_mask_==false";
      }
    }
    break;
  case Caffe::CPU:
...
template <typename Dtype>
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape, bool enable_mask) {
  if (reshape) {
  ...
  if ( !enable_mask_ && (proto.double_mask_size()>0 || proto.mask_size()>0) ) {
    LOG(INFO) << "FromProto: ignore mask in input proto, because enable_mask_==false";
    //mask_.reset(new SyncedMemory(capacity_ * sizeof(Dtype)));
    //enable_mask_ = true;
  }
  if (enable_mask_) {
