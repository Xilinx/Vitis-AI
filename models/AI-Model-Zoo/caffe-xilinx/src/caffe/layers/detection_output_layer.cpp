...
void DetectionOutputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  has_segmap_ = detection_output_param.has_segmap();
...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  //CHECK_EQ(num_priors_ * num_loc_classes_ * 6, bottom[0]->channels())
  ...
  // [image_id, label, confidence, xmin, ymin, xmax, ymax, angle_cos, angle_sin]
...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  if(has_segmap_) {
    const Dtype* seg_data = bottom[3]->cpu_data();
    Dtype* output_seg_data = top[1]->mutable_cpu_data();
  ...
  if(ori_flag)
     GetLocOriPredictions(loc_data, num, num_priors_, num_loc_classes_,
    ...
    for (map<int, vector<int> >::iterator it = all_indices[i].begin();
      ...
      for (int j = 0; j < indices.size(); ++j) {
        ...
        if(ori_flag){
          ...
          top_data[count * 9 + 7] = bbox.angle_cos();
          top_data[count * 9 + 8] = bbox.angle_sin();
            }
        else{
