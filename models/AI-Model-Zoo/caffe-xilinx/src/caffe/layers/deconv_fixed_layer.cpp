...
#include "caffe/layers/deconv_fixed_layer.hpp"
#include "caffe/util/quantize.hpp"

namespace caffe {

template <typename Dtype>
void DeconvolutionFixedLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

  // Set up deconvolution layer
  ...
  // Set up deconvolution layer for fixed forward
  LayerParameter layer_param(this->layer_param_);
  layer_param.set_type("Deconvolution");
  fixed_forward_deconv_layer_ = LayerRegistry<Dtype>::CreateLayer(layer_param);
  fixed_forward_deconv_layer_->SetUp(bottom, top);
}

template <typename Dtype>
void DeconvolutionFixedLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

  DeconvolutionLayer<Dtype>::Reshape(bottom, top);
  fixed_forward_deconv_layer_->Reshape(bottom, top);
}

template <typename Dtype>
void DeconvolutionFixedLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype> *> &bottom, const vector<Blob<Dtype> *> &top) {

  auto fixed_blobs = fixed_forward_deconv_layer_->blobs();
  if (!enable_fix_) {
    ...
    fixed_forward_deconv_layer_->Forward(bottom, top);
    return;
  }
...
template <typename Dtype>
void DeconvolutionFixedLayer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype> *> &top, const vector<bool> &propagate_down,
    const vector<Blob<Dtype> *> &bottom) {

  DeconvolutionLayer<Dtype>::Backward_cpu(top, propagate_down, bottom);
}

#ifdef CPU_ONLY
STUB_GPU(DeconvolutionFixedLayer);
#endif

INSTANTIATE_CLASS(DeconvolutionFixedLayer);
REGISTER_LAYER_CLASS(DeconvolutionFixed);
