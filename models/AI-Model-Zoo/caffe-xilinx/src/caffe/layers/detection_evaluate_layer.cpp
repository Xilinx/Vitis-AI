...
void DetectionEvaluateLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  bool ori_flag = false;
  if(bottom[0]->width() == 7 && bottom[1]->width() == 8) ori_flag =false; 
  else if (bottom[0]->width() == 9 && bottom[1]->width() == 10) ori_flag = true; 
  else LOG(FATAL) <<"demension out of the input";
