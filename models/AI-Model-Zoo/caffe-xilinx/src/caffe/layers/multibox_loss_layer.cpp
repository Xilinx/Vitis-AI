...
void MultiBoxLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  ori_flag = multibox_loss_param.ori_flag();
  ...
  if(ori_flag) loc_shape.push_back(6);
  else loc_shape.push_back(4);
...
void MultiBoxLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if(ori_flag){
  CHECK_EQ(num_priors_ * loc_classes_ * 6, bottom[0]->channels())
...
void MultiBoxLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(ori_flag)
    GetLocOriPredictions(loc_data, num_, num_priors_, loc_classes_, share_location_,
                      &all_loc_preds);
  else
  ...
  if (num_matches_ >= 1) {
    ...
    loc_shape[0] = 1;
    if(ori_flag) loc_shape[1] = num_matches_ * 6;
    else loc_shape[1] = num_matches_ * 4;
    loc_pred_.Reshape(loc_shape);
    loc_gt_.Reshape(loc_shape);
      ...
      for (int i = 0; i < num_; ++i) {
        for (map<int, vector<int> >::iterator it =
          ...
          for (int j = 0; j < match_index.size(); ++j) {
            ...
            int start_idx ;
            if(ori_flag){
            start_idx = loc_classes_ * 6 * j + label * 6;
            caffe_copy<Dtype>(6, loc_pred_diff + count * 6,
                              loc_bottom_diff + start_idx);
              }
            else {
            start_idx = loc_classes_ * 4 * j + label * 4;
