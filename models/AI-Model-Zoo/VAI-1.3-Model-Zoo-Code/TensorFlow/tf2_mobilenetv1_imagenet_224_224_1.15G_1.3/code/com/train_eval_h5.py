...
flags.DEFINE_string(
    'model', './h5_model/mobilenet_1_0_224_tf.h5',
...
def main():
  ...
  if FLAGS.dump or FLAGS.quantize_eval:
      ...
      model = MobileNet(alpha=1.0, weights=FLAGS.model)
  ...
  if FLAGS.quantize:
      # do quantization
      from tensorflow_model_optimization.quantization.keras import vitis_quantize
      model = vitis_quantize.VitisQuantizer(model).quantize_model(calib_dataset=imagenet_seq, replace_relu6=False)
  ...
  if FLAGS.dump:
      # do quantize dump
      vitis_quantize.VitisQuantizer.dump_model(model, imagenet_seq, FLAGS.dump_output_dir)
