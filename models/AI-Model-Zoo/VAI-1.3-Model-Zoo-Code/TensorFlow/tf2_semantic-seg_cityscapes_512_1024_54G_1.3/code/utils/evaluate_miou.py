...
def parse_args():
    ...
    parser.add_argument('--ignore_label', type=int, default=19, help='dataset classes')
...
def evaluate_segmentation(gt_dir, result_dir, num_classes, ignore_label, result_suffix, gt_suffix, result_file):
    ...
    for key in sorted(gt_dict.keys()):
        ...
        #print(gt_id_set)
        prediction = np.asanyarray(Image.open(result_path))
        #print(np.unqiue(prediction))
    ...
    if ignore_label is not None:
        gt_id_set.remove(int(ignore_label))
        print("Ignore the label id {} ".format(int(ignore_label)))
    print('GT id set', gt_id_set)
    ious = per_class_iu(hist) * 100
    miou = np.mean(ious[list(gt_id_set)])
   
    print('mean IoU(%): {:.2f}'.format(miou))
    print('per-class IoU(%): ' )
    print(','.join(['{:.2f}'.format(n) for n in ious]))
...
def main():
    ...
    evaluate_segmentation(args.gt, args.result, args.num_classes, args.ignore_label, args.result_suffix, args.gt_suffix, args.result_file)
