...
'''How to organize coco dataset folder:
 coco2017/
       ...
       |->val2017.txt
'''
tf.app.flags.DEFINE_string('dataset_directory', '../data/EDD/',
                           'All datas directory')
tf.app.flags.DEFINE_string('train_splits', 'train',
                           'Comma-separated list of the training data sub-directory')
tf.app.flags.DEFINE_string('validation_splits', 'val',
                           'Comma-separated list of the validation data sub-directory')
tf.app.flags.DEFINE_string('output_directory', '../data/EDD/tfrecords',
...
def _find_image_bounding_boxes(directory, cur_record, height, width):
    ...
    Returns:
      ...
      truncated: List of ints indicate the truncation of that bounding box.
    """
    anna_file = os.path.join(directory, 'bbox', cur_record[1].replace('jpg', 'txt'))
    ...
    for anna_line in anna_lines:
        ...
        print(items)    
        label = items[-1]
        labels.append(int(dataset_common.EDD_LABELS[label][0]))
        ...
        '''
        bboxes.append((float(items[1]) / height,
                       float(items[0]) / width,
                       float(items[3]) / height,
                       float(items[2]) / width))
...
def main(unused_argv):
    ...
    _process_dataset('edd_val', FLAGS.dataset_directory, parse_comma_list(FLAGS.validation_splits), FLAGS.validation_shards)
    _process_dataset('edd_train', FLAGS.dataset_directory, parse_comma_list(FLAGS.train_splits), FLAGS.train_shards)
