...
if [ $phase_type = "train" ];then
   ...
   caffe_exec caffe train -solver $solver_file -gpu 2 2>&1 | tee train.log
else
   echo "now is for testing"
   model_file='./float/trainval.prototxt'
   model_weights=./'float/trainval.caffemodel'
   caffe_exec caffe test -model $model_file -weights $model_weights -ssd 11point -iterations 10000 -gpu 0 2>&1 | tee test.log
fi
