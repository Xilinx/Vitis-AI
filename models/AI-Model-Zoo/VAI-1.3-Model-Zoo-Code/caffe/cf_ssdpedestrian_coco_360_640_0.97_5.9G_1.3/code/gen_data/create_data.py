...
def convert_lmdb(image_root, convert_file, db_file, height, width):
    ...
    cmd = 'python ' + caffe_py_path + ' --label-type=%s --anno-type=%s --label-map-file=%s --min-dim=%d --max-dim=%d --resize-width=%d --resize-height=%d --check-label %s %s %s %s ../../data/link_640_360/' %\
