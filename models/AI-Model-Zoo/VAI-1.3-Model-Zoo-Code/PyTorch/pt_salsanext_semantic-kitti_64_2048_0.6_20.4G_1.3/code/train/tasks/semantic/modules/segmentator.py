...
class SalsaNet(nn.Module):
    def __init__(self, ARCH, nclasses, path=None, path_append="", strict=False):
        ...
        self.downCntx = ResContextBlock(5, 16)
        self.resBlock1 = ResBlock(16, 16, 0.2, pooling=True, drop_out=False)
        self.resBlock2 = ResBlock(16, 2 * 16, 0.2, pooling=True)
        self.resBlock3 = ResBlock(2 * 16, 4 * 16, 0.2, pooling=True)
        self.resBlock4 = ResBlock(4 * 16, 8 * 16, 0.2, pooling=True)
        self.resBlock5 = ResBlock(8 * 16, 16 * 16, 0.2, pooling=True)
        self.resBlock6 = ResBlock(16 * 16, 16 * 16, 0.2, pooling=False)

        self.upBlock1 = UpBlock(16 * 16, 16 * 16, 0.2)
        self.upBlock2 = UpBlock(16 * 16, 8 * 16, 0.2)
        self.upBlock3 = UpBlock(8 * 16, 4 * 16, 0.2)
        self.upBlock4 = UpBlock(4 * 16, 2 * 16, 0.2)
        self.upBlock5 = UpBlock(2 * 16, 16, 0.2, drop_out=False)

        self.logits = nn.Conv2d(16, nclasses, kernel_size=(1, 1))
