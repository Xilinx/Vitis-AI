...
from onnxruntime.quantization.quant_utils import QuantizedValue, QuantizedValueType, attribute_to_kwarg, ms_domain
...
class QLinearActivation(QuantOperatorBase):
    ...
    def QuantizeClipRelu(self):
        ...
        if node.input[
                0] not in self.quantizer.quantized_value_map or self.quantizer.is_activation_symmetric:
    ...
    def quantize(self):
        ...
        (
            ...
            _,
            _,
        ) = self.quantizer._get_quantization_params(node.output[0], use_scale,
                                                    use_zeropoint)
        (
