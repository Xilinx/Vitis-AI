...
class VitisAIONNXQuantizer(ONNXQuantizer):
    ...
    def __init__(
        ...
        self.is_weight_symmetric = (is_weight_int8 if "WeightSymmetric"
                                    not in self.extra_options else
                                    self.extra_options["WeightSymmetric"])
        ...
        self.activation_qType = (onnx_proto.TensorProto.INT8
                                 if activation_qType == QuantType.QInt8 else
                                 onnx_proto.TensorProto.UINT8)
        self.weight_qType = (onnx_proto.TensorProto.INT8
                             if weight_qType == QuantType.QInt8 else
                             onnx_proto.TensorProto.UINT8)
        """
    ...
    def quantize_node_with_sub_graph(self, node):
        ...
        graph_attrs = [
            ...
            if attr.type == onnx.AttributeProto.GRAPH or
            attr.type == onnx.AttributeProto.GRAPHS
        ]
        if len(graph_attrs) == 0:
            return node
        node_name = node.name if node.name != "" else "{}_node_count_{}".format(
            node.op_type, len(self.new_nodes))
        kwargs = {}
        for attr in node.attribute:
            ...
            kwargs.update(kv)
        return onnx.helper.make_node(node.op_type,
                                     node.input,
                                     node.output,
                                     name=node.name,
                                     **kwargs)
    ...
    def check_opset_version(self):
        ai_onnx_domain = [
            opset for opset in self.model.model.opset_import
    ...
    def has_QDQ_nodes(self):
        ...
        """
        return any(node.op_type == "QuantizeLinear" or
    ...
    def is_float_tensor(self, tensor_name):
        ...
        if tensor_name in self.value_infos.keys():
            ...
            if vi.type.HasField(
                    "tensor_type"
            ) and vi.type.tensor_type.elem_type == onnx_proto.TensorProto.FLOAT:
                return True
    ...
    def _get_dynamic_input_quantization_params_int8(self, input_name,
        ...
        # Zero point
        initializer_zp = onnx.helper.make_tensor(self.fixed_zero_zp_name, qType,
                                                 [], [0])
    ...
    def _get_quantization_params(self,
        ...
        if use_scale is None or use_zeropoint is None:
            if self.quantization_params is None or param_name not in self.quantization_params:
                logging.info(
                    'Quantization parameters for tensor:"{}" not specified'.
                    format(param_name))
                return False, "", "", "", ""
        ...
        init_scale = onnx.helper.make_tensor(scale_name,
                                             onnx_proto.TensorProto.FLOAT,
    ...
    def _get_quantize_input_nodes(self,
                                  node,
                                  input_index,
                                  qType,
                                  given_scale_name=None,
                                  given_zp_name=None):
        """
        ...
        if (given_scale_name is not None) and (given_zp_name is not None):
            data_found, scale_name, zp_name = (True, given_scale_name,
                                               given_zp_name)
        else:
    ...
    def contains_tensor(self, tensor_name):
        ...
        """
        return ((tensor_name in self.value_infos) or
                (tensor_name in self.tensor_names) or
                (tensor_name in self.generated_value_names))
    ...
    def calculate_quantization_params(self):
        ...
        for node in self.model.nodes():
            ...
            if len(self.model.input_name_to_nodes()[node.input[0]]) != 1:
                continue
            if node.input[0] not in self.tensors_range.keys(
        ...
        for tensor_name in self.tensors_range.keys():
            ...
            qmin, qmax = get_qmin_qmax_for_qType(
                self.activation_qType, symmetric=self.is_activation_symmetric)
