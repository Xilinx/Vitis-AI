...
Status RemoveNodes(const GraphDef &input_graph_def,
  ...
  if (!context.params.count("op")) {
    return errors::InvalidArgument("remove_nodes expects at least one 'op'"
        ...
        TF_RETURN_IF_ERROR(ReplaceMatchingOpTypes(
            ...
            [&inputs_to_rename, &required_nodes, &any_nodes_removed](
              ...
              if (required_nodes.count(replace_node.name())) {
                DLOG_INFO(1)
                    << "Skipping replacement for " << replace_node.name();
...
REGISTER_DECENT_Q_GRAPH_TRANSFORM("remove_nodes", RemoveNodes);
