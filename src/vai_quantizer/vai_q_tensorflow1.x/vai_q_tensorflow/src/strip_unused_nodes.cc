...
Status ShapeForPlaceholder(const TransformFuncContext &context,
  ...
  *result = {};
  ...
  if (context.params.count("shape")) {
    ...
    TF_RETURN_IF_ERROR(PartialTensorShapeFromString(shape_string, result));
  }
...
// Get required nodes between input nodes and output nodes
Status GetRequiredNodes(const GraphDef &input_graph_def,
                        const std::unordered_set<string> &input_nodes,
                        const std::unordered_set<string> &output_nodes,
                        std::unordered_set<string> *required_nodes) {
  for (const string &input : input_nodes) {
    required_nodes->insert(NodeNameFromInput(input));
  }
  for (const string &output : output_nodes) {
  ...
  for (const string &output_node : output_nodes) {
    current_inputs.push_back(output_node);
  }
...
Status StripUnusedNodes(const GraphDef &input_graph_def,
  ...
  std::unordered_set<string> input_nodes;
  std::unordered_set<string> output_nodes;
  ...
  for (const string &output : context.output_names) {
    output_nodes.insert(NodeNameFromInput(output));
  }

  std::unordered_set<string> required_nodes;
  GetRequiredNodes(input_graph_def, input_nodes, output_nodes, &required_nodes);
...
REGISTER_DECENT_Q_GRAPH_TRANSFORM("strip_unused_nodes", StripUnusedNodes);
