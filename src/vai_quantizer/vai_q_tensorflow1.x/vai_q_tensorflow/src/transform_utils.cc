...
void PrintNodeMatch(const NodeMatch &match, const int indent) {
  ...
  std::cout << match.node.name() << "(" << match.node.op() << ")" << std::endl;
  for (auto input : match.inputs) {
    PrintNodeMatch(input, indent + 2);
  }
  return;
}
...
bool GraphMatcher::DoesOpTypeMatch(
  ...
  VLOG(1) << "Looking at node " << node.name();
...
Status ReplaceMatchingOpTypes(
  ...
  for (const NodeMatch &match : matches) {
    // LOG(WARNING) << " debug match head  " << match.node.name();
...
Status PartialTensorShapeFromString(const string &shape_string,
  ...
  *result = PartialTensorShape(dims);
  return Status::OK();
}
