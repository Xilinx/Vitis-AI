...
from onnx import helper, numpy_helper, shape_inference, AttributeProto, TensorProto, onnx_pb
...
class Graph(object):
    ...
    def make_const(self, name, np_val, skip_conversion=False, raw=True):
        ...
        if raw and not is_bytes:
            if np_val.dtype not in tf1_numpy_known_datatype:
                if np_val.dtype == 'bfloat16':
                    onnx_tensor = tf_utils.tf_to_onnx_tensor_bfloat16(np_val, name)
            else: 
        ...
        if np_val.dtype not in tf1_numpy_known_datatype:
            if np_val.dtype == 'bfloat16':
                self.set_dtype(name, onnx_pb.TensorProto.BFLOAT16)
        else:
