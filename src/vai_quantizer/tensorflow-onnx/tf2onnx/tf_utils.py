...
TF_TO_ONNX_DTYPE = {
    ...
    types_pb2.DT_BFLOAT16: onnx_pb.TensorProto.BFLOAT16,
...
def tf_to_onnx_tensor_bfloat16(np_data, name=""):
    tensor = TensorProto()
    tensor.dims.extend(np_data.shape)
    if name:
        tensor.name = name
    tensor.data_type = TensorProto.BFLOAT16
    tensor.raw_data = np_data.tobytes()  # note: tobytes() is only after 1.9.
    if sys.byteorder == 'big':
    # Convert endian from big to little
        tensor.raw_data = np.frombuffer(tensor.raw_data, dtype=np_data.dtype).byteswap().tobytes()
    return tensor


def tf_to_onnx_tensor(tensor, name=""):
    """Convert tensorflow tensor to onnx tensor."""
    tf1_numpy_known_datatype =['float16','float32','float64','int8','int16','int32','int64','uint8','uint16','uint32','uint64','bool']
    np_data = get_tf_tensor_data(tensor)
    if np_data.dtype not in tf1_numpy_known_datatype:
        if np_data.dtype == 'bfloat16':
            return tf_to_onnx_tensor_bfloat16(np_data, name)
    ...
    while progress:
        ...
        for node in ops:
            ...
            can_fold = node.type not in ['Enter', 'Placeholder', 'PlaceholderWithDefault', 'Switch', 'Merge',
                                         ...
                                         'QuantizeAndDequantizeV4', 'FakeQuantWithMinMaxVars', 'FakeQuantWithMinMaxVarsPerChannel']
