...
class deephi_Conv3d(torch.nn.modules.conv.Conv3d):
  r"""DeePhi Conv3d operation, support float and double"""

  def __init__(self, *args, **kwards):
    super(deephi_Conv3d, self).__init__(*args, **kwards)
  ...
  def fake_quantize_forward(self, input):
    ...
    output = torch.nn.functional.conv3d(qinput,
                                        weight = qweight,
                                        bias = qbias,
                                        stride = self.stride,
    ...
    if NndctOption.nndct_param_corr.value > 0:
      ...
      if (self.quant_mode == 1 and not self.stop):
        res_f = torch.nn.functional.conv3d(input,
                                           self.weight_bak,
                                           bias = self.bias_bak,
                                           stride = self.stride,
                                           padding = self.padding,
...
def Conv3d(*args, **kwargs):
  ...
  return deephi_Conv3d(*args, **kwargs)
