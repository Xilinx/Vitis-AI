...
class OptPass(object):
  ...
  def slice_to_strided_slice(raw_graph, node_sets):
    ...
    for nodeset in sorted_node_list:
      ...
      if node.inputs[0].node is None or (node.inputs[0].node.kind != "strided_slice" or (node.inputs[0].node in slice_end_point)):
        ...
        strided_node.owning_graph = raw_graph.owning_graph
        strided_node.add_input(node.inputs[0])
        strided_node.dtype = node.dtype
        strided_node.scope_name = node.scope_name
        strided_node.source_range = node.source_range
  ...
  def merge_matmul_with_add(raw_graph, node_sets):
    ...
    for node_set in nodes_list:
      if not all([node.inputs[1].name in raw_graph.owning_graph.param_names() for node in node_set]):
        continue
  ...
  def remove_reduantant_view(raw_graph, node_sets):
    ...
    for nodeset in nodes_list:
      view = nodeset[0]
      if view.inputs[0].shape is None or view.outputs[0] is None:
        continue
      if view.inputs[0].shape == view.outputs[0].shape:
        ...
        for i, inp in enumerate(view.flatten_inputs):
          if i == 0 or inp.node is None or len(inp.node.out_nodes) > 1:
            continue
          remove_nodes.append(inp.node)
  ...
  @staticmethod
  def merge_internal_type_as_new(raw_graph, node_sets):
    nodes_list = OptPass.merge_node_sets_to_list(node_sets)
    if not nodes_list:
      return
