...
# different tf version the input_signature have different name regulation
# if: tf_version < 2.11: arg_name + '/' + key_id   e.g 'args_0/0', 'args_0/1'
# if: tf_version >= 2.11: arg_name + '_' + key_id   e.g 'args_0_0', 'args_0_1'
_input_signature_suffix_pattern = '/' if tf_utils.is_tf_version_less_than('2.11') else '_'
...
class GraphCodeGenerator(object):
  ...
  def __init__(self, graph, class_spec):
    ...
    # alias for object names to be written.
    self._obj_name_alias = {}
  ...
  def _get_unique_name(self, obj):
    """Get unique name for given object. If there are objects with the same
    name, generate alias names for them.
    """
    if obj in self._obj_name_alias:
      alias = self._obj_name_alias[obj]
    else:
      # Use alias to avoid duplicate obj names.
      alias = obj.__name__
      while alias in self._obj_name_alias.values():
        alias = '_' + alias
      self._obj_name_alias[obj] = alias
    return alias

  def _generate_obj_import(self, obj):
    module_name = tf_inspect.getmodule(obj).__name__
    obj_name = obj.__name__
    obj_unique_name = self._get_unique_name(obj)

    if obj_unique_name == obj_name:
      return 'from {} import {}'.format(module_name, obj_unique_name)
    else:
      return 'from {} import {} as {}'.format(module_name, obj_name,
                                              obj_unique_name)

  def generate_class_def(self):
    class_def = 'class {cls_name}({base_cls}):\n'.format(
        cls_name=self._class_spec.name, base_cls=self._class_spec.base.__name__)
  ...
  def _init_string(self, entity):
    ...
    return '{obj_name}({args})'.format(
        obj_name=self._get_unique_name(entity.obj), args=', '.join(args))
  ...
  def generate_call_fn(self, fn_name):
    ...
    def arg_retriving_path(arg, path=()):
      ...
      See https://github.com/tensorflow/tensorflow/blob/r2.3/tensorflow/python/util/nest.py::_yield_sorted_items
    ...
    for i, arg in enumerate(self._input_signature):
      for path in list(arg_retriving_path(arg)):
        ...
        arg_name = _input_signature_suffix_pattern.join(str(s) for s in arg_name_path)
...
class GraphTranslator(object):
  ...
  """
  if tf_utils.is_tf_version_greater_equal('2.6'):
    rescaling_layer = keras_layers.Rescaling
    normalization_layer = keras_layers.Normalization
  else:
    rescaling_layer = keras_layers.experimental.preprocessing.Rescaling
    normalization_layer = keras_layers.experimental.preprocessing.Normalization
  _op_to_layer = {
      OpTypes.BATCH_NORM:
  ...
  _op_to_func = {
      ...
      OpTypes.RFFT: fft_ops.rfft,
      OpTypes.COMPLEX_ABS: tf.abs,
      OpTypes.ANGLE: tf.math.angle,
      OpTypes.CAST: tf.cast,
      OpTypes.EXP: tf.math.exp,
      OpTypes.IRFFT: fft_ops.irfft,
      OpTypes.PAD: tf.pad,
      OpTypes.TRANSPOSE: tf.transpose,
      OpTypes.SUM: tf.math.reduce_sum,
      OpTypes.RESHAPE: tf.reshape,
