...
from tf_nndct.quantization import TFLstmQstrategy
...
class TFQuantizer(BaseQuantizer):
  ...
  def __init__(self,
    ...
    self.quant_param = DPUQConfigImp()
    self.quant_strategy = TFLstmQstrategy(quant_config, self.quant_param)
  ...
  def create_fp_tensor(self, name, fp_name, tensor_type='output'):
    ...
    if self.quant_mode == 2:
      fp = self.get_quant_config(fp_name, False, tensor_type)
      init_val = fp[1]
  ...
  def get_fp_and_quantize(self,
    ...
    if tensor_type != 'param':
      mth = 4
    bnfp = self.get_quant_config(fp_name, False, tensor_type)
    bw = bnfp[0]
    if self.quant_mode == 1:
      ...
      if tensor_type != 'param':
        ...
        #                              fp_stat_tensor))
        self.config_history[tensor_type][fp_name][0].append(bnfp[1])
        data = np.array(self.config_history[tensor_type][fp_name][0])
        bnfp[1] = stats.mode(data)[0][0]
        bnfp[1] = bnfp[1].astype(np.int32).tolist()
        fp_tensor.assign(bnfp[1])
      bnfp = self.set_quant_config(fp_name, bnfp, tensor_type)
    ...
    if self.lstm:
      ...
      for node in self.Nndctgraph.nodes:
        if (node.name.split('/')[-1] == 'mul_2'):
          output_pos = self.get_quant_config(node.name, False)
      ...
      for node in self.Nndctgraph.nodes:
        ...
        if (node.name.split('/')[0] == 'rnn_cell_0' and
          ...
          self.set_quant_config(node.name, output_pos)
