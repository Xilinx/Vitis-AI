...
from tensorflow_model_optimization.python.core.quantization.keras.vitis.common import vitis_quantize_wrapper_v2 as vitis_quantize_wrapper
...
from tensorflow_model_optimization.python.core.quantization.keras.vitis.layers import vitis_multi_head_attention
...
class LayersQuantize(transforms.Transform):
  """Quantize layers in the quantize support list, by wrapping them with QuantizeWrappersV2.
  Special layers like InputLayer and Convolution + BatchNormalization will be handled by
  other transformations.

  Layer => QuantizeWrapperV2(Layer inside)
  """
  ...
  def __init__(self, input_model, quantize_registry, mode):
    ...
    self.quant_layers = [
        ...
        'GlobalAveragePooling2D', 'Vitis>VitisDFRelu', 'Activation', 'Add', 'LayerNormalization' 
    ]
  ...
  def replacement(self, match_layer):
    ...
    skip_layers = [
        'InputLayer', 'Vitis>VitisQuantize', 'Vitis>QuantizeWrapperV2', 'Vitis>QuantizeWrapper',
    ...
    quant_layer = vitis_quantize_wrapper.QuantizeWrapperV2(
  ...
  def custom_objects(self):
    ...
    objs.update({
        ...
        'QuantizeWrapper':
            vitis_quantize_wrapper.QuantizeWrapperV2,
        'VitisQuantize':
...
class MultiHeadAttentionQuantize(transforms.Transform):
  """Quantize MultiHeadAttention layers, by wrapping them with VitisMultiHeadAttentionQuantize.

  MultiHeadAttention => VitisMultiHeadAttentionQuantize
  """

  def __init__(self, input_model, quantize_registry):
    super(MultiHeadAttentionQuantize, self).__init__()
    self.quantize_registry = quantize_registry
    self.input_model = input_model

  def pattern(self):
    return LayerPattern('MultiHeadAttention', {}, [])

  def replacement(self, match_layer):
    multi_head_attention_layer = self.input_model.get_layer(match_layer.layer['config']['name'])
    num_heads = multi_head_attention_layer._num_heads
    key_dim = multi_head_attention_layer._key_dim
    quantize_config = None
    if self.quantize_registry.supports(multi_head_attention_layer):
    ...
    quant_multi_head_attention_layer = vitis_multi_head_attention.VitisMultiHeadAttentionQuantize(
      multi_head_attention_layer,
      quantize_config
      )

    quant_multi_head_attention_weights = match_layer.weights
    quant_multi_head_attention_layer_node = LayerNode.from_layer(
      quant_multi_head_attention_layer, weights=quant_multi_head_attention_weights)
    return quant_multi_head_attention_layer_node
...
class ConvBNQuantizeFold(transforms.Transform):
  ...
  VitisConvBNQuantize => QuantizeWrapperV2(Conv)
  """
