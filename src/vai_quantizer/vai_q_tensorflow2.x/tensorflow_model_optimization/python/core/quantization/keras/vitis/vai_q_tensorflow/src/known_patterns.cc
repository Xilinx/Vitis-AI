...
// BatchMatMulV2
const OpTypePattern batchmatmul_pattern(
    {"BatchMatMulV2",
      {
        input_pattern, // input node_0
        input_pattern, // input node_1
      }
    });
DEFINE_GET_INPUT_NODES(BatchMatMul) {
  std::vector<const NodeDef*> input_nodes;
  input_nodes.push_back(&(match.inputs[0].node));
  input_nodes.push_back(&(match.inputs[1].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(BatchMatMul) {

  return std::vector<const NodeDef*> ();
}
const BatchMatMulPattern batchmatmul_pattern_wrapper(batchmatmul_pattern, "batchmatmul");

// BatchMatMulV2 && truediv
const OpTypePattern batchmatmul_truediv_pattern(
    {"Mul",
      {
        batchmatmul_pattern,
        weight_pattern,
...
DEFINE_GET_INPUT_NODES(BatchMatMulTrueDiv) {
  std::vector<const NodeDef*> input_nodes;
  input_nodes.push_back(&(match.inputs[0].inputs[0].node));
  input_nodes.push_back(&(match.inputs[0].inputs[1].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(BatchMatMulTrueDiv) {

  return std::vector<const NodeDef*> ();
}
const BatchMatMulTrueDivPattern batchmatmul_truediv_pattern_wrapper(batchmatmul_truediv_pattern, "batchmatmul_truediv");
...
// MatMul + Reshape + bias for  Dense
const OpTypePattern dense_pattern(
    {"BiasAdd",
      {
        {"Reshape",
          {
...
DEFINE_GET_INPUT_NODES(Dense) {
  std::vector<const NodeDef*> input_nodes;
  //BiasAdd->Reshape->ConcatV2->GatherV2->Shape->AddV2
  //input_nodes.push_back(&(match.inputs[0].inputs[1].inputs[0].inputs[0].inputs[0].node));
  //BiasAdd->Reshape->MatMul->Reshape->input
  input_nodes.push_back(&(match.inputs[0].inputs[0].inputs[0].inputs[0].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(Dense) {
  std::vector<const NodeDef*> weights_nodes;
  //BiasAdd->Reshape->MatMul->bias
  weights_nodes.push_back(&(match.inputs[0].inputs[0].inputs[1].node));
  //BiasAdd->bias
  weights_nodes.push_back(&(match.inputs[1].node));
  return weights_nodes;
}
const DensePattern dense_pattern_wrapper(dense_pattern, "dense");
...
//Gelu
const OpTypePattern gelu_pattern(
    {"Mul",
      {
        {"Mul",
          {
...
DEFINE_GET_INPUT_NODES(Gelu) {
  std::vector<const NodeDef*> input_nodes;
  input_nodes.push_back(&(match.inputs[0].inputs[1].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(Gelu) {
  return std::vector<const NodeDef*> ();
}
const GeluPattern gelu_pattern_wrapper(gelu_pattern, "gelu");
...
// LayerNorm
const OpTypePattern layernorm_pattern(
    {"AddV2", // add node
      {
        {"Mul" },
        {"Sub",
...
DEFINE_GET_INPUT_NODES(Layernorm) {
  std::vector<const NodeDef*> input_nodes;
  //AddV2->Sub->Mul->Mean->input tensor
  input_nodes.push_back(&(match.inputs[1].inputs[1].inputs[0].inputs[0].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(Layernorm) {
  std::vector<const NodeDef*> weights_nodes;
  //AddV2->Sub->value
  weights_nodes.push_back(&(match.inputs[1].inputs[0].node));
  //AddV2->Sub->Mul->Mul->weights
  weights_nodes.push_back(&(match.inputs[1].inputs[1].inputs[1].inputs[1].node));
  return weights_nodes;

}
const LayernormPattern layernorm_pattern_wrapper(layernorm_pattern, "layernorm");
...
//AddV2
const OpTypePattern add_v2_pattern(
    {"AddV2",
      {
...
DEFINE_GET_INPUT_NODES(Add_v2) {
  std::vector<const NodeDef*> input_nodes;
  input_nodes.push_back(&(match.inputs[1].node));
  return input_nodes;
}
DEFINE_GET_WEIGHTS_NODES(Add_v2) {
  return std::vector<const NodeDef*> ();
}
const Add_v2Pattern add_v2_pattern_wrapper(add_v2_pattern, "add_v2");
...
const OpTypePattern other_pattern(
    {"Maximum|Max|AvgPool|MaxPool|Mean|Pad|MirrorPad|Concat|ConcatV2|Squeeze|ExpandDims|Relu|Relu6|AddN|Sum"});
