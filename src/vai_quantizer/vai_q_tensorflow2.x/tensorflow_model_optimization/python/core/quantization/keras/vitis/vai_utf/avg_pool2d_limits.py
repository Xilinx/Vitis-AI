...
# ==============================================================================
"""AveragePooling2D layer limit classes."""
...
@register_keras_serializable(package='Vitis', name='AvgPool2DLimits')
class AvgPool2DLimits(BaseLayerLimits):

  def __init__(self, dpu_target, limits={}):
    """Init with dpu target."""
    super(AvgPool2DLimits, self).__init__(limits)
  ...
  def is_supported(self):
    ...
    return PoolType.Value('avg') in self.pool_engine.pool_type

  def build_attr_limits(self):
    """Build attr limits."""
    # avg kernel_size
    avg_kernel_size_limit = limit_utils.str_to_pair_limit(
        self.pool_engine.avg_limit.kernel_size)
    self.add_attr_limit('pool_size', avg_kernel_size_limit)
    # avg strides
    avg_strides_limit = limit_utils.str_to_pair_limit(
        self.pool_engine.avg_limit.stride)
    self.add_attr_limit('strides', avg_strides_limit)
  ...
  def in_limits(self, layer):
    ...
    # attr limits
    is_in_attr_limit, attr_msgs = super(AvgPool2DLimits,
  ...
  def in_act_limits(self, layer, act_layer):
    ...
    # Only Pool + Activation|ReLU|LeakyReLU will go into this function now.
