...
# ==============================================================================
"""Vitis dataformat transformations pipeline for quantization."""
...
from tensorflow_model_optimization.python.core.quantization.keras.vitis.quantize_strategy.bfp import vitis_bfp_quantize_transforms
...
class VitisBFPOptimizeTransformsPipeline(TransformsPipeline):
  """Vitis datafromat pre-quantization optimize model transformations."""

  def apply(self, model, candidate_layers, layer_metadata):
    """Implement vitis datafromat optimize transforms to make it more quantize-friendly.
    ...
    if configs['train_with_bn']:
      logger.info('Have not implemented VitisConvBN for BFP quantization...')
      #transforms = [
...
class VitisBFPQuantizeTransformsPipeline(TransformsPipeline):
  """Vitis dataformat model quantize transformations pipeline."""

  def apply(self, model, candidate_layers, layer_metadata, quantize_registry,
            mode):
    """Implement vitis dataformat quantize transforms.
    ...
    available_annotate_transforms = collections.OrderedDict({
        'no_quant_in_conv_bn_act':
            vitis_bfp_quantize_transforms.NoQuantInConvBNAct(),
        'no_quant_in_conv_act':
            vitis_bfp_quantize_transforms.NoQuantInConvAct(
                pre_annotated_model, quantize_registry),
        'no_quant_in_add_act':
            vitis_bfp_quantize_transforms.NoQuantInAddAct(
                pre_annotated_model, quantize_registry),
    })
    ...
    quantize_transforms = [
        vitis_bfp_quantize_transforms.InputLayerQuantize(quantize_registry,
                                                        mode),
        vitis_bfp_quantize_transforms.LayersQuantize(annotated_model,
                                                    quantize_registry, mode),
        vitis_bfp_quantize_transforms.MultiHeadAttentionQuantize(annotated_model, quantize_registry)
    ]
...
class VitisBFPFinalizeTransformsPipeline(TransformsPipeline):
  """Vitis dataformat model finalize transformations pipeline."""
