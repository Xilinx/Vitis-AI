...
NodeConfigMap LocateDense(const NodeMatch &match,
                                   const QuantizeConfig &config,
                                   std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &biasadd_node = match.node;
  const NodeDef &reshape_node = match.inputs[0].node;
  const NodeDef &bias_node = match.inputs[1].node;
  const NodeDef &matmul_node = match.inputs[0].inputs[0].node;
  const NodeDef &concat_node = match.inputs[0].inputs[1].node;
  ...
  if (!CheckDtype(matmul_node)) return ops_to_quantize;

  bool updated = UpdateNodeConfigMap(
      weight_node, GetWtConfig(config, matmul_node.op()), ops_to_quantize);
  updated = updated && UpdateNodeConfigMap(bias_node, GetWtConfig(config),
                                           ops_to_quantize);
  updated = updated && UpdateNodeConfigMap(biasadd_node, GetActConfig(config),
                                           ops_to_quantize);
  if (updated) {
    DLOG_INFO(1) << "Quantize Dense: " << matmul_node.name();
    node_groups.insert(std::vector<string>{
        matmul_node.name(), input_node.name(),
        reshape_node.name(), weight_node.name(), bias_node.name()});
  }
  return ops_to_quantize;
}
...
NodeConfigMap LocateBatchMatMul(const NodeMatch &match, const QuantizeConfig &config,
                          std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &batchmatmul_node = match.node;
  if (!CheckDtype(batchmatmul_node)) return ops_to_quantize;

  bool updated =
      UpdateNodeConfigMap(batchmatmul_node, GetActConfig(config), ops_to_quantize);
  if (updated) {
    DLOG_INFO(1) << "Quantize BatchMatMul: " << batchmatmul_node.name() << "("
                 << batchmatmul_node.op() << ")";
  }
  return ops_to_quantize;
}

NodeConfigMap LocateBatchMatMulTrueDiv(const NodeMatch &match, const QuantizeConfig &config,
                          std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &truediv_node = match.node;
  if (!CheckDtype(truediv_node)) return ops_to_quantize;

  bool updated =
      UpdateNodeConfigMap(truediv_node, GetActConfig(config), ops_to_quantize);
  if (updated) {
    DLOG_INFO(1) << "Quantize BatchMatMul&TrueDiv: " << truediv_node.name() << "("
                 << truediv_node.op() << ")";
  }
  return ops_to_quantize;
}
...
NodeConfigMap LocateGelu(const NodeMatch &match,
                              const QuantizeConfig &config,
                              std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &gelu_node = match.node;
  if (!CheckDtype(gelu_node)) return ops_to_quantize;

  bool updated = UpdateNodeConfigMap(gelu_node, GetActConfig(config),
                                           ops_to_quantize);
  if (updated) {
    DLOG_INFO(1) << "Quantize Gelu: " << gelu_node.name();
  }
  return ops_to_quantize;
}
...
NodeConfigMap LocateLayerNorm(const NodeMatch &match,
                              const QuantizeConfig &config,
                              std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &add_node = match.node;
  ...
  const NodeDef &mul_node_1 = match.inputs[1].inputs[1].node;
  const NodeDef &sub_node_weights = match.inputs[1].inputs[0].node;
  const NodeDef &mean_node = match.inputs[1].inputs[1].inputs[0].node;
  const NodeDef &mul_node_2 = match.inputs[1].inputs[1].inputs[1].node;
  const NodeDef &mul_node_2_weights = match.inputs[1].inputs[1].inputs[1].inputs[1].node;
  const NodeDef &rsqrt_node = match.inputs[1].inputs[1].inputs[1].inputs[0].node;
  const NodeDef &add_node_2 = match.inputs[1].inputs[1].inputs[1].inputs[0].inputs[0].node;
  const NodeDef &mean_node_2 = match.inputs[1].inputs[1].inputs[1].inputs[0].inputs[0].inputs[0].node;

  if (!CheckDtype(mul_node)) return ops_to_quantize;

  bool updated =
      UpdateNodeConfigMap(sub_node_weights, GetWtConfig(config), ops_to_quantize);
  updated = updated & UpdateNodeConfigMap(mul_node_2_weights, GetWtConfig(config),
                                          ops_to_quantize);
  updated = updated & UpdateNodeConfigMap(add_node, GetActConfig(config),
                                          ops_to_quantize);
  if (updated) {
    DLOG_INFO(1) << "Quantize layernorm: " << add_node.name() << " <-- "
                 << mul_node.name() << "(" << mul_node.op() << ")";
    node_groups.insert(std::vector<string>{add_node.name(), mul_node.name(),
                                           sub_node.name(), mul_node_1.name(),
                                           mean_node.name(), mul_node_2.name(),
                                           rsqrt_node.name(), add_node_2.name(),mean_node_2.name()});
  }
  return ops_to_quantize;
}
...
NodeConfigMap LocateAdd_v2(const NodeMatch &match, const QuantizeConfig &config,
                          std::set<NodeGroup> &node_groups) {
  NodeConfigMap ops_to_quantize;
  const NodeDef &add_node = match.node;
  const NodeDef &const_node = match.inputs[1].node;
  if (!CheckDtype(add_node)) return ops_to_quantize;

bool updated = UpdateNodeConfigMap(add_node, GetActConfig(config), ops_to_quantize);
  updated = updated & UpdateNodeConfigMap(const_node, GetActConfig(config),
                                          ops_to_quantize);

  if (updated) {
    DLOG_INFO(1) << "Quantize add const: " << add_node.name() << "("
                 << add_node.op() << ")  +  " << const_node.name() << "("
                 << const_node.op() << ")";
  }
  return ops_to_quantize;
}
...
Status GraphQuantizer::_LocateOpsToQuantize(const GraphDef &input_graph_def) {
  ...
  std::map<string, LocationFuncHandle> map_pattern_func({
      ...
      {"convfc", &LocateConvfc},
      {"batchmatmul", &LocateBatchMatMul},
      {"batchmatmul_truediv", &LocateBatchMatMulTrueDiv},
  ...
  for (auto match_id = 0; match_id < _matched_node_patterns.size();
   ...
   //     if (!CheckDtype(concat_node)) {
   //                 DLOG_WARNING << "As the datatype, it cannot be quantized"
