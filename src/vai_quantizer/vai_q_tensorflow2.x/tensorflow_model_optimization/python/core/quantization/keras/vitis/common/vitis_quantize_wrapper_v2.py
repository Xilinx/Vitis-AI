...
"""Wrapper which applies quantization operations over underlying layer.
   ...
   `QuantizeWrapperV2` is responsible for modifying the construction of the
...
@register_keras_serializable(package='Vitis', name='QuantizeWrapperV2')
class QuantizeWrapperV2(tf.keras.layers.Wrapper):
  ...
  def __init__(self, layer, quantize_config, mode, **kwargs):
    ...
    super(QuantizeWrapperV2, self).__init__(layer, **kwargs)
  ...
  def build(self, input_shape):
    ...
    if hasattr(self.quantize_config, 'get_input_and_quantizers'):
      for input_id, quantizer in self.quantize_config.get_input_and_quantizers(self.layer):
        quantizer_vars = quantizer.build(input_shape, 'input_'+str(input_id), self)
    ...
    self._bias_vars = []
    for bias, quantizer in \
        self.quantize_config.get_biases_and_quantizers(self.layer):
      # In case of layers without bias
      if bias is None:
        continue

      quantizer_vars = quantizer.build(bias.shape, self._weight_name(bias.name),
                                       self)

      self._bias_vars.append((bias, quantizer, quantizer_vars))
      # Needed to ensure unquantized biases get trained as part of the wrapper.
    ...
    for output_id, quantizer in self.quantize_config.get_output_and_quantizers(
        self.layer):
  ...
  def get_quantize_info(self):
    ...
    for input_id, quantizer, quantizer_vars in self._input_quantizer_vars:
      input_str = 'input_' + str(input_id)
      quantize_info[input_str] = {'type': 'input', 'info': quantizer.get_quantize_info()}
    ...
    #(TODO) Separate weight and bias in quantize_info
    for bias, quantizer, quantizer_vars in self._bias_vars:
      b_name = bias.name.lstrip('quant_')
      quantize_info[b_name] = {
          'type': 'weight',
          'info': quantizer.get_quantize_info()
      }
  ...
  def set_quantize_info(self, new_quantize_info):
    for input_id, quantizer, quantizer_vars in self._input_quantizer_vars:
      input_str = 'input_' + str(input_id)
      if input_str in new_quantize_info:
        quantizer.set_quantize_info(new_quantize_info[input_str]['info'])
      else:
        logger.warning('input quantize_info not found in new quantize_info {}'.format(new_quantize_info))
    ...
    for bias, quantizer, quantizer_vars in self._bias_vars:
      b_name = bias.name.lstrip('quant_')
      if b_name in new_quantize_info:
        quantizer.set_quantize_info(new_quantize_info[b_name]['info'])
      else:
        logger.warning(
            'Bias quantize_info of {} not found in new_quantize_info {}'.format(
                b_name, new_quantize_info))
  ...
  def call(self, inputs, training=None):
    ...
    if self._input_quantizer_vars:
      input_id, input_quantizer, input_quantizer_vars = self._input_quantizer_vars[0]
      # Handle layers with multiple outputs
      if isinstance(inputs, list) or isinstance(inputs, tuple):
        quantized_inputs = inputs
        for input_id in range(len(inputs)):
          quantized_inputs[input_id] = common_utils.smart_cond(
              training,
              self._make_quantizer_fn(input_quantizer, inputs[input_id], True,
                                      self.mode, input_quantizer_vars),
              self._make_quantizer_fn(input_quantizer, inputs[input_id], False,
                                      self.mode, input_quantizer_vars))
        inputs = quantized_inputs
      else:
        inputs = common_utils.smart_cond(training, 
            self._make_quantizer_fn(input_quantizer, inputs, True, self.mode, input_quantizer_vars),
            self._make_quantizer_fn(input_quantizer, inputs, False, self.mode, input_quantizer_vars))
    ...
    for unquantized_bias, quantizer, quantizer_vars in self._bias_vars:
      quantized_bias = common_utils.smart_cond(
          training,
          self._make_quantizer_fn(quantizer, unquantized_bias, True, self.mode,
                                  quantizer_vars),
          self._make_quantizer_fn(quantizer, unquantized_bias, False, self.mode,
                                  quantizer_vars))
    ...
    if isinstance(outputs, list) or isinstance(outputs, tuple):
      ...
      for output_id, output_quantizer, output_quantizer_vars in self._output_quantizer_vars:
        quantized_outputs[output_id] = common_utils.smart_cond(
            ...
            self._make_quantizer_fn(output_quantizer, outputs[output_id], False,
                                    self.mode, output_quantizer_vars))
  ...
  def get_config(self):
    base_config = super(QuantizeWrapperV2, self).get_config()
    config = {
  ...
  def from_config(cls, config):
    ...
    # QuantizeWrapperV2 may be constructed with any QuantizeConfig and the
  ...
  def trainable_weights(self):
    total_trainable_weights = self._trainable_weights
    exist_names = []
    for w in self._trainable_weights:
      exist_names.append(w.name)
    for w in self.layer.trainable_weights:
      if w.name not in exist_names:
        total_trainable_weights.append(w)
    return total_trainable_weights
