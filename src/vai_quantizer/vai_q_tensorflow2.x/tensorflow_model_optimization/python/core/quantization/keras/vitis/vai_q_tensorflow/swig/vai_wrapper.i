...
void DecentQCheckGraphWithStringInputs(string graph_def_string, string graph_path) {
  ...
  if (!check_status.ok()) {
    decent_q::Set_TF_Status_from_Status(out_status, check_status);
    return;
  }
...
string DecentQConvertConstantsToVariablesWithStringInputs(string graph_def_string,
  ...
  if (!config_status.ok()) {
    decent_q::Set_TF_Status_from_Status(out_status, config_status);
    return "";
  }
  ...
  tensorflow::Status create_status =
      graph_quantizer.ConvertConstantsToVariables(input_graph_def, 
          output_graph_def);
  if (!create_status.ok()) {
    decent_q::Set_TF_Status_from_Status(out_status, create_status);
    return "";
  }
