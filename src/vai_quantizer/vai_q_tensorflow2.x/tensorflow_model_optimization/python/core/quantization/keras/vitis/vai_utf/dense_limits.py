...
@register_keras_serializable(package='Vitis', name='DenseLimits')
class DenseLimits(BaseLayerLimits):

  def __init__(self, dpu_target, limits={}):
    """Init with dpu target."""
    super(DenseLimits, self).__init__(limits)
  ...
  def in_limits(self, layer):
    ...
    # attr limits
    is_in_attr_limit, attr_msgs = super(DenseLimits, self).in_attr_limits(layer)
    # other limits
  ...
  def in_act_limits(self, conv_layer, act_layer):
    ...
    if isinstance(act_layer, keras.layers.Activation):
      ...
      if act_type_str not in supported_acts_str:
        is_in_limit = False
        msgs.append(
            'Dense<{}> not supported, supported act types are {}'.format(
      ...
      if act_layer.max_value and act_layer.max_value != 6.0:
        is_in_limit = False
        msgs.append(
            'Dense<ReLU(max_value={})> is not supported, supported act types are {}'
      ...
      if act_type not in supported_acts:
        ...
        is_in_limit = False
        msgs.append(
            'Dense<LeakyReLU(alpha={})> is not supported, only alpha=0.1 is supported'
