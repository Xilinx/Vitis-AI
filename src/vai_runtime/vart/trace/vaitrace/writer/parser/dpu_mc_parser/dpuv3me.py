...
class dpuv3meInstParser(dpuMcParserBase):
    def __init__(self):
        name = "DPUCAHX8L_ISA0"
        super().__init__(name)
        self.opcode_table = [
            self.inst_desc("LOAD", 0b0000, 7),
            self.inst_desc("SAVE", 0b0100, 4),
            self.inst_desc("CBLOAD", 0b0101, 3),
            self.inst_desc("CONVINIT", 0b1001, 5),
            self.inst_desc("CONV", 0b1000, 7),
            self.inst_desc("DWINIT", 0b1011, 4),
            self.inst_desc("DPTWISE", 0b1010, 6),
            self.inst_desc("POOLINIT", 0b0110, 2),
            self.inst_desc("POOL", 0b1100, 5),
            self.inst_desc("ELEWINIT", 0b1101, 3),
        ...
        while (pos < len(mc)):
            ...
            if inst_name == "LOAD":
                ...
                if load_bank_id < 16:
                    ...
                    inst_load_para_size = load_len * load_jr + load_channel
