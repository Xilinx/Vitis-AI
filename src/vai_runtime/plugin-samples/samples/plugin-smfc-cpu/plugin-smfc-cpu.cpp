...
#include "plugin-smfc-cpu/plugin-smfc-cpu.hpp"

std::string plugin_smfc_cpu::get_plugin_name() { return "plugin_smfc_cpu"; }

std::string plugin_smfc_cpu::get_device() { return "SM-CPU"; }

std::map<std::string, std::string> plugin_smfc_cpu::get_runner() {
  return {
    {"run", "libvart-softmax-runner-cpu.so"},
    {"sim", "libvart-softmax-runner-cpu.so"},
    {"ref", "libvart-softmax-runner-cpu.so"},
  };
}

std::set<xir::Subgraph*> plugin_smfc_cpu::partition(xir::Graph* graph) {
...
void plugin_smfc_cpu::compile(xir::Subgraph* subgraph) {
