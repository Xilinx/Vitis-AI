...
std::string _output_dir ("yolo_outputs");
...
{
  auto nboxes = results.front()->get_tensor()->get_element_num() / 7;
  if(!_output_dir.empty()) {
    ...
    for(int b = 0; b < img_paths.size(); ++b) {
      ...
      for (int box = 0; box < nboxes; ++box) {
        if (boxptr[box*7] == b) {
          float class_id = boxptr[(box*7)+1];
          float score    = boxptr[(box*7)+2];
          float x        = boxptr[(box*7)+3];
          float y        = boxptr[(box*7)+4];
          float w        = boxptr[(box*7)+5];
          float h        = boxptr[(box*7)+6];
...
// Yolo-v3 Object Detection Inference using
...
{  
  ...
  // Load graph
  sysMan->loadGraphs("graph_zoo/graph_yolov3_dpucadf8h_proteus.json");
  ...
  for (int i = 0; i < images.size(); i+=bt) {
    ...
    for (int b = 0; b < bt; ++b) {
      ...
      auto tensorOut = xir::Tensor::create(
                         "imread_output", shape, 
