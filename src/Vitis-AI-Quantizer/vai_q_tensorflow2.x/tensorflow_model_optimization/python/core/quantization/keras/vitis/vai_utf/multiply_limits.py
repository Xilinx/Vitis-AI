...
@register_keras_serializable(package='Vitis', name='MultiplyLimits')
class MultiplyLimits(BaseLayerLimits):

  def __init__(self, dpu_target, limits={}):
    """Init with dpu target."""
    super(MultiplyLimits, self).__init__(limits)
  ...
  def is_supported(self):
    ...
    return ElewType.Value('mult') in self.eltwise_engine.elew_type
  ...
  def in_limits(self, layer):
    ...
    # attr limits
    is_in_attr_limit, attr_msgs = super(MultiplyLimits,
  ...
  def in_act_limits(self, mul_layer, act_layer):
    ...
    if isinstance(act_layer, keras.layers.Activation):
      ...
      if act_type_str not in supported_acts_str:
        is_in_limit = False
        msgs.append(
            'Multiply<{}> not supported, supported act types are {}'.format(
      ...
      if act_layer.max_value and act_layer.max_value != 6.0:
        is_in_limit = False
        msgs.append(
            'Multiply<ReLU(max_value={})> is not supported, supported act types are {}'
      ...
      if act_type not in supported_acts:
        ...
        is_in_limit = False
        msgs.append(
            'Multiply<LeakyReLU(alpha={})> is not supported, only alpha=0.1 is supported'
