...
class QatProcessor(object):
  ...
  def __init__(self, model, inputs, bitwidth=8, mix_bit=False, device=None):
    ...
    for name, group in self._quant_config.items():
      ...
      for key in group:
        ...
        quant_method = None
        if (not name
            == TensorTypes.PARAM) and NndctOption.nndct_ip_v70_bert_qat.value:
          specil_method_op = [OpTypes.LAYER_NORM, OpTypes.SOFTMAX]
          if self._graph.node(node).op.type in specil_method_op:
            quant_method = 4

        tqt_quantizer = tqt_mod.TQTQuantizer(
            get_bitwidth(qinfo), tensor_type, method=quant_method)
  ...
  def to_deployable(self, trained_model, output_dir):
    ...
    logging.info(f'Saving deployable model to {saved_path}')
  ...
  def export_onnx_model(self,
                        ...
                        check_model=False, 
                        opt_graph=False):
    self._qprocessor.export_onnx_model(output_dir, verbose, dynamic_batch, opset_version,
                                       native_onnx, dump_layers, check_model, opt_graph)
