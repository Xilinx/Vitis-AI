...
struct OpenPoseAccThread : public AccThread {
  ...
  virtual int run() override {
    ...
    while(1){
      ...
      LOG_IF(INFO, 1)
      <<"test openpsoe, queue size: "<<queue_->size()
      <<" dpu_result id: "<<dpu_result.frame_id; 
      if(g_last_frame_id == int(dpu_result.frame_id)){
        of << "]";
        of.close();
        LOG(INFO)
        <<"test openpsoe accuracy done! byebye "<<queue_->size();
        exit(0);
      }
    }
    return 0;
...
int main(int argc, char *argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [model] {
        return vitis::ai::OpenPose::create(model);
      },
      vitis::ai::OpenPoseAccThread::instance(),
      2);
}
