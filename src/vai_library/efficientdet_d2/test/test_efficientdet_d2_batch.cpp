...
int main(int argc, char* argv[]) {
  ...
  auto results = detector->run(batch_images);
  ...
  for (auto batch_idx = 0u; batch_idx < results.size(); batch_idx++) {
    ...
    for (auto& box : results[batch_idx].bboxes) {
      ...
      std::cout << "RESULT-" << batch_idx << ": " << label << "\t" << xmin
                << "\t" << ymin << "\t" << xmax << "\t" << ymax << "\t"
                << confidence << "\n";
      rectangle(batch_images[batch_idx], Point(xmin, ymin), Point(xmax, ymax),
                Scalar(0, 255, 0), 1, 1, 0);
    }
    std::cout << std::endl;
    std::string save_name = batch_images_names[batch_idx].substr(
        0, batch_images_names[batch_idx].size() - 4);
    cv::imwrite(save_name + "_result.jpg", batch_images[batch_idx]);
  }
