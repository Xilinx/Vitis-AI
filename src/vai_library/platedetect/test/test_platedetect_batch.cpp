...
int main(int argc, char *argv[]) {
  ...
  for (size_t i = 0; i < batch; i++) {
    auto rect = cv::Rect{(int)(nres[i].box.x * batch_images[i].cols),
                         (int)(nres[i].box.y * batch_images[i].rows),
                         (int)(nres[i].box.width * batch_images[i].cols),
                         (int)(nres[i].box.height * batch_images[i].rows)};
