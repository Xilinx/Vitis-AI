...
std::unique_ptr<MedicalSegmentation> MedicalSegmentation::create(
    const std::string &model_name, bool need_mean_scale_process) {
  return std::unique_ptr<MedicalSegmentation>(
      new MedicalSegmentationImp(model_name, need_mean_scale_process));
}
