...
def check_env_args():
  global INSTALL, DEVELOP, BDIST, EMIT_WARNING
...
def build_config_setup():
  ...
  if not BDIST:
    ...
    cpp_src_path = os.path.join(cwd, "pytorch_nndct/nn/src")
    source_files = []
    ...
    ext_module = CUDAExtension(name='pytorch_nndct.nn._kernels', 
                               language='c++',
