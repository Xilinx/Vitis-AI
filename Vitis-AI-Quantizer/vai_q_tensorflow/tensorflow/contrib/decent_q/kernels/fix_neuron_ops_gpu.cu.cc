...
T l2_diff_gpu(const int n, const T* x, const T* y, const int method,
  ...
  l2_diff_kernel_gpu<T><<<block_count, thread_per_block>>>(n, x, y, res_gpu,
                                                           method, mode);
  float sum;
...
template <typename T>
void FixNeuronFunctor<GPUDevice, T>::operator()(
    const GPUDevice& d, const T* input_tensor, const int& bit_width,
    const int& method, const int& mode, const int& phase, int& saved_pos,
    int& iter, std::map<int, int>& pos_hist, T* output_tensor,
    const int& size) {
  ...
  if (phase == 2 || phase == 0) {
    ...
    if (method == 0) {
      ...
      current_pos = dp_get_quantize_pos_diffs_gpu(size, input_tensor, bit_width,
                                                  method, mode);
    } else {
