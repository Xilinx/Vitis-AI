...
def save_prototxt(net_info, protofile, region=True):
    ...
    def format_value(value):
        if is_number(value):
            ...
            return '\"%s\"' % value
    def print_block(block_info, prefix, indent, fp):
        blanks = ''.join([' ']*indent)
        #print >>fp, '%s%s {' % (blanks, prefix)
        print('{}{} {}'.format(blanks, prefix, '{'), file=fp)
        for key,value in block_info.items():
            if type(value) == OrderedDict:
                print_block(value, key, indent+4, fp)
            elif type(value) == list:
                for v in value:
#                    #print >> fp, '%s    %s: %s' % (blanks, key, format_value(v))
                    print('{}    {}: {}'.format(blanks, key, format_value(v)), file=fp)
            else:
#                #print >> fp, '%s    %s: %s' % (blanks, key, format_value(value))
                print('{}    {}: {}'.format(blanks, key, format_value(value)),file=fp)
#
#        #print >> fp, '%s}' % blanks
        print(' {}{}'.format(blanks, '}'), file=fp)
    props = net_info['props']
    layers = net_info['layers']
   # print >> fp, 'name: \"%s\"' % props['name']
    print('name: "{}"' .format(props['name']), file=fp)
    print('layer {}'.format('{'),file=fp)
    print()
   # print >> fp, 'input: \"%s\"' % props['input']
    print('name: "{}"' .format(props['input']), file=fp)
    print('type: "{}"' .format("Input"), file=fp)
    #print('input: "{}"' .format(props['input']), file=fp)
    print('top: "{}"' .format(props['input']), file=fp)
    print('input_param: {} shape: {} dim: {} dim: {} dim: {} dim: {} {} {}'.format('{', '{', props['input_dim'][0], props['input_dim'][1], props['input_dim'][2],props['input_dim'][3], '}', '}'),file=fp)
    print('{}'.format('}'),file=fp)
    #print >> fp, 'input_dim: %s' % props['input_dim'][0]
    #print('input_dim: {}' .format(props['input_dim'][1]), file=fp)
    #print('input_dim: {}' .format(props['input_dim'][2]), file=fp)
    #print('input_dim: {}' .format(props['input_dim'][3]), file=fp)
