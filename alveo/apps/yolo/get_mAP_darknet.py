...
#        print("file:",name_list[file_num], " has ", len(list_detections_gt), " gt lines and ",  len(list_detections), " detections")
    ...
    for class_num in range(num_classes):
        ...
        if map_points==0: # mAP calculation: ImageNet, PascalVOC 2010-2012
            last_recall = PR_info_list_of_lists[len(detection_list) - 1][class_num].recall
            last_precision = PR_info_list_of_lists[len(detection_list) - 1][class_num].precision
            for rank in range(len( detection_list)-2, 0, -1):
                delta_recall = last_recall - PR_info_list_of_lists[rank][class_num].recall;
                last_recall = PR_info_list_of_lists[rank][class_num].recall;
                if (PR_info_list_of_lists[rank][class_num].precision > last_precision):
                    last_precision = PR_info_list_of_lists[rank][class_num].precision;
...
def main():
    ...
    parser.add_argument('--prob_threshold', type=float, default=0.25, help='threshold for calculation of f1 score')
    parser.add_argument('--points', type=int, default=0, help='point 0 (AUC) for ImageNet, PascalVOC 2010-2012, your custom dataset `points 101` for MS COCO and `points 11` for PascalVOC 2007')
    ...
    mAP = calc_detector_mAP(args_dict['detection_labels'], args_dict['ground_truth_labels'], len(class_names), class_names, args_dict['prob_threshold'], args_dict['IOU_threshold'], args_dict['points'])
