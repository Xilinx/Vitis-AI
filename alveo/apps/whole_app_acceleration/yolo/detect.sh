...
usage() {
  ...
  echo -e ""
  echo "Arguments"
  echo "  -t TEST, --test TEST          Mode of execution. Possible values: [test_detect, test_detect_jpeg]"
  echo "  -m MODEL, --model MODEL       Yolo model variant. Possible values: [ "
  echo "                                                                  yolo_v3_spp, tiny_yolo_v3]"
...
SUPPORTED_MODES="test_detect, test_detect_jpeg"
SUPPORTED_MODELS="yolo_v3_spp, tiny_yolo_v3"
...
if [[ ( -v NETWORK_HEIGHT ) && ( -v NETWORK_WIDTH ) ]]; then
  ...
  echo "python ${VAI_ALVEO_ROOT}/apps/yolo/modify_network_dims.py --input_deploy_file $NET_DEF --output_deploy_file work/new_dim.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH"
  python ${VAI_ALVEO_ROOT}/apps/yolo/modify_network_dims.py --input_deploy_file $NET_DEF --output_deploy_file work/new_dim.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH
  if [[ $? != 0 ]]; then echo "Network modification failed. Exiting ..."; exit 1; fi;
  NET_DEF=work/new_dim.prototxt

  echo "python ${VAI_ALVEO_ROOT}/apps/yolo/modify_network_dims.py --input_deploy_file $NET_DEF_FPGA --output_deploy_file work/new_dim_fpga.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH"
  python ${VAI_ALVEO_ROOT}/apps/yolo/modify_network_dims.py --input_deploy_file $NET_DEF_FPGA --output_deploy_file work/new_dim_fpga.prototxt --in_shape 3 $NETWORK_HEIGHT $NETWORK_WIDTH
  if [[ $? != 0 ]]; then echo "Network modification failed. Exiting ..."; exit 1; fi;
...
if [ "$TEST" == "test_detect" ]; then
  TEST=test_detect_vitis_pp.py
elif [ "$TEST" == "test_detect_jpeg" ]; then
  TEST=test_detect_vitis.py
else
  echo "ERROR : -t|--test should be test_detect | test_detect_jpeg>. Exiting ..."
  exit 1
fi
