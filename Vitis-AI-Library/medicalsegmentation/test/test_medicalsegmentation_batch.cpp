...
int main(int argc, char *argv[]) {
  ...
  for (auto batch_idx = 0u; batch_idx < result.size(); batch_idx++) {
     ...
     for(int i=0; i<5; i++) {
         std::string fname("results/"+classTypes[i]+"/"+filenamepart1+".png");
         cv::resize(result[batch_idx].segmentation[i], img_save, batch_images_size[batch_idx], 0, 0, cv::INTER_LINEAR);
