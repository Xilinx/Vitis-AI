...
class MedicalSegmentationImp : public vitis::ai::TConfigurableDpuTask<MedicalSegmentation> {
public:
  MedicalSegmentationImp(const std::string &model_name, bool need_preprocess = true);
  virtual ~MedicalSegmentationImp();

private:
  virtual MedicalSegmentationResult run(const cv::Mat &img) override;
  virtual std::vector<MedicalSegmentationResult> run(const std::vector<cv::Mat> &img) override;
