    ...
    for(int i = 2;i< argc;i++){
     ...
     cv::resize(image, img_resize, cv::Size(width, height), 0, 0,
      ...
      cout << "res.box.score " << res.box.score << " " //
           << "res.box.x " << res.box.x << " toint "<< (int)(res.box.x * image.cols) << " "                   //
           << "res.box.y " << res.box.y << " toint " << (int)(res.box.y * image.rows)<< " "                   //
           << "res.box.width " << res.box.width<< " toint "<< (int)(res.box.width * image.cols) << " "           //
           << "res.box.height " << res.box.height << " toint "<< (int)(res.box.height * image.rows) << std::endl
           << "The real coordinate is: xx: "         //
           << "res.top_left :("  << res.top_left.x << " , "  << res.top_left.y << ") "//
           << "res.top_right :("  << res.top_right.x << " , "  << res.top_right.y << ") "//
           << "res.bottom_left :("  << res.bottom_left.x << " , "  << res.bottom_left.y << ") "//
           << "res.bottom_right :("  << res.bottom_right.x << " , "  << res.bottom_right.y << ") "//
           << std::endl;
