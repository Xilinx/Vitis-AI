...
struct PhysicalMemory {
    ...
    Tp *PhyAddr(uintptr_t addr) {
        if(!(addr >= phy_base) ||
           !(addr < phy_base + length)){
            cerr << "out of range " << hex
                 << "addr = " << "0x" << addr << " "
                 << "phy_base = " << "0x" << phy_base << " "
                 << "lenght = " << "0x" << length << " "
                 << endl;
            assert(false);
        }
        ...
        while (c < size) {
            ...
            *p32 = *from4;
            ...
            for (unsigned int i = start; i < sizeof(uint32_t) && c < size; ++i, ++c) {
                int ch = fgetc(fp);
                if (ch == EOF) {
                    std::cerr << "unexpected file lenght. " << hex
                              << "phy addr = 0x" << addr << " "
                              << "size = 0x " << size << " "
                              << "c = 0x" << c << std::endl;
                    exit(1);
...
struct MemRegValue {
    ...
    _Tp operator=(_Tp v) {
        ...
        _Tp ret = (*pointer_ = v);
        wmb();
        if(0) cout <<  __FILE__ << ":" << __LINE__ << ": [" << __FUNCTION__<< "] "
             << "write ret = " << hex << "0x" << ret << dec << "@" << (void*)pointer_
             << endl;
        return ret;
    }
    _Tp read() {
        ...
        wmb();
        if(0) cout <<  __FILE__ << ":" << __LINE__ << ": [" << __FUNCTION__<< "] "
             << "read ret = " << hex << "0x" << ret << dec << "@" << (void*)pointer_
             << endl;
        return ret;
    }
