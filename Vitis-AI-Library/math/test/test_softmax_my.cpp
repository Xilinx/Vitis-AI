...
#define __TOC__(tag)                                             \
  ...
  cout << #tag << " : "                                          \
       << std::chrono::duration_cast<std::chrono::microseconds>( \
              __##tag##_end_time - __##tag##_start_time).count() << endl;
...
void print_s8x8(int8x8_t d) {
    std::cout << (int)vget_lane_s8(d, 7) << ", "
       << (int)vget_lane_s8(d, 6) << ", "
       << (int)vget_lane_s8(d, 5) << ", "
       << (int)vget_lane_s8(d, 4) << ", "
       << (int)vget_lane_s8(d, 3) << ", "
       << (int)vget_lane_s8(d, 2) << ", "
       << (int)vget_lane_s8(d, 1) << ", "
       << (int)vget_lane_s8(d, 0) << std::endl;
}

void print_u8x8(uint8x8_t d) {
    std::cout << (int)vget_lane_u8(d, 7) << ", "
       << (int)vget_lane_u8(d, 6) << ", "
       << (int)vget_lane_u8(d, 5) << ", "
       << (int)vget_lane_u8(d, 4) << ", "
       << (int)vget_lane_u8(d, 3) << ", "
       << (int)vget_lane_u8(d, 2) << ", "
       << (int)vget_lane_u8(d, 1) << ", "
       << (int)vget_lane_u8(d, 0) << std::endl;
}

void print_f32x4(float32x4_t d){
    std::cout << vgetq_lane_f32(d, 3) << ", "
              << vgetq_lane_f32(d, 2) << ", "
              << vgetq_lane_f32(d, 1) << ", "
              << vgetq_lane_f32(d, 0)
              << std::endl;
...
static inline float32x4_t char2float(const uint16x4_t d0_l , const float step, const float32x4_t f_miny){
 ...
 static void __attribute__((noinline)) softmax4_internal(const int8_t *input, float scale, unsigned int group, float *output){
  unsigned int batch = group / 8;
  // __TIC__(expTable)
  // ExpTable expTable(scale);
  Table table = getTable(std::abs((int)log2(scale)));
  register float miny = table.miny_;
  register float step = table.step_;
  register uint8x8x4_t q_table_ = table.char_table_;
  if(0){
  ...
  register float32x4_t f_miny = vdupq_n_f32(miny);
  // __TOC__(expTable)
...
static void softmax8_internal(const int8_t *input, float scale, unsigned int group, float *output){
