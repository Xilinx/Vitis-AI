...
class SegmentationImp : public vitis::ai::TConfigurableDpuTask<Segmentation> {
 ...
 private:
  ...
  virtual std::vector<SegmentationResult> run_8UC1(const std::vector<cv::Mat>& images) override;
  virtual SegmentationResult run_8UC3(const cv::Mat& image) override;
  virtual std::vector<SegmentationResult> run_8UC3(const std::vector<cv::Mat>& images) override;
};
