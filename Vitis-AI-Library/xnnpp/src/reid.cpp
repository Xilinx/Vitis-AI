...
//# DPUV1 needs input data as float
  ...
  auto bn_means = vector<float>(config.reid_param().bn_means().begin(), config.reid_param().bn_means().end());
  auto bn_weights = vector<float>(config.reid_param().bn_weights().begin(), config.reid_param().bn_weights().end());
  auto bn_vars = vector<float>(config.reid_param().bn_vars().begin(), config.reid_param().bn_vars().end());
  //std::cout<<bn_means.size()<<" "<<bn_weights.size()<<" "<<bn_vars.size()<<" "<<width<<" "<<height<<" "<<channels<<std::endl;
  float a[channels];
  float sum;
