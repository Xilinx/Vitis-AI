...
SSDdetector::SSDdetector(unsigned int num_classes, // int background_label_id,
    ...
    : num_classes_(num_classes),
      ...
      keep_top_k_(keep_top_k), confidence_threshold_(confidence_threshold),
      nms_top_k_(nms_top_k), nms_threshold_(nms_threshold), eta_(eta),
      priors_(priors), scale_(scale), clip_(clip) {
...
void SSDdetector::Detect(const T *loc_data, const float *conf_data,
  ...
  for (size_t label = 1; label < indices.size(); ++label) {
    for (size_t idx : indices[label]) {
      ...
      result->emplace_back(label, score, box_rect, bbox[4], bbox[5]);
    }
  }
...
template <typename T>
float SSDdetector::JaccardOverlap(const T (*bboxes)[6], int idx, int kept_idx,
                                  bool normalized) {
  /*
    if (decoded_bboxes_.find(idx) == decoded_bboxes_.end()) {
  ...
  */
  ...
  return intersect_size <= 0
             ? 0
             : intersect_size / (bbox1[6] + bbox2[6] - intersect_size);
}

template float SSDdetector::JaccardOverlap(const int (*bboxes)[6], int idx,
                                           int kept_idx, bool normalized);
template float SSDdetector::JaccardOverlap(const int8_t (*bboxes)[6], int idx,
                                           int kept_idx, bool normalized);

template <typename T>
void SSDdetector::DecodeBBox(const T (*bboxes)[6], int idx, bool normalized) {
  vector<float> bbox(7, 0);
  // scale bboxes
