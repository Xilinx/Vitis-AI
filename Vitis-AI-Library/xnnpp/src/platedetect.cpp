...
static PlateDetectResult FilterBox(float scale_box, float scale_conf, int input_width, int input_height,
...
std::vector<PlateDetectResult> plate_detect_post_process(
  ...
  for (auto i = 0u; i < batch_size; i++) {
    results.emplace_back(FilterBox(vitis::ai::library::tensor_scale(output_tensors[0][0]),
    vitis::ai::library::tensor_scale(output_tensors[0][1]), 
    input_tensors[0][0].width, input_tensors[0][0].height, 
    (int8_t*)output_tensors[0][0].get_data(i), (int8_t*)output_tensors[0][1].get_data(i))); 
  }
  return results; 
}
