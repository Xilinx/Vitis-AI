...
void Predict::findLocalmaximum(vector<int>& datase, vector<cv::Point_<int>>& seed) {
...
void Predict::cluster(const vector<int> &outImage, vector<int> &clusters){
    ...
    for(int i = indexs.size()-1; i > -1; i--){
        ...
        for(int j = indexs.size()-1; j >-1; j--){
            if(indexs[j].size() == 2 && abs(indexs[i][0] - indexs[j][0]) <= range_x && abs(indexs[i][1] - indexs[j][1]) <= range_y)
...
void Predict::voteClassOfClusters(const vector<int> &datase, const vector<int> &clusters, vector<int> &types){
    ...
    for(int i = 0; i < ipm_height_; i++) {
        for(int j = 0; j <ipm_width_; j++) {
            if (clusters[i * ipm_width_ + j] > 0)
            {
                ...
                tempClass[clusters[i * ipm_width_ + j]].push_back(datase[i * ipm_width_ + j]);
            }
        }
        }
    ...
    for(size_t i=0; i<tempClass.size(); i++){
        if(tempClass.size()>0)
            types[i] = majorityElement(tempClass[i]);
    }
}
