...
int main(int argc, char *argv[]) {
  ...
  for (auto batch_idx = 0u; batch_idx < batch; batch_idx++) {    
    ...
    auto ratio =
        std::min(static_cast<float>(width) / static_cast<float>(batch_images[batch_idx].cols),
                 static_cast<float>(height) / static_cast<float>(batch_images[batch_idx].rows));
    ...
    std::cout << "new_w " << new_w << " " //
         ...
         << "ratio " << ratio << " " //
         << std::endl;
    cv::resize(batch_images[batch_idx], img_resize, new_size, 0, 0, cv::INTER_NEAREST);
