...
PoseDetectImp::PoseDetectImp(const std::string &model_name,
    ...
    : vitis::ai::TConfigurableDpuTask<PoseDetect>(model_name,
                                                   need_preprocess) {}
...
PoseDetectResult PoseDetectImp::run(const cv::Mat &input_image) {
  ...
  vitis::ai::globalAvePool(
      (int8_t *)configurable_dpu_task_->getOutputTensor()[0][0].get_data(0), 184, 4, 7,
  ...
  auto ret = vitis::ai::pose_detect_post_process(configurable_dpu_task_->getInputTensor(), configurable_dpu_task_->getOutputTensor(), image.size());
  return ret[0];
}

std::vector<PoseDetectResult> PoseDetectImp::run(const std::vector<cv::Mat> &input_images) {
  ...
  configurable_dpu_task_->run(0);
  __TOC__(POSEDETECT_DPU_CONV)
  for (auto i = 0u; i < configurable_dpu_task_->getOutputTensor()[0][0].batch; i++) {
    vitis::ai::globalAvePool(
      (int8_t *)configurable_dpu_task_->getOutputTensor()[0][0].get_data(i), 184, 4, 7,
      (int8_t *)configurable_dpu_task_->getInputTensor()[1][0].get_data(i), 8);
  }
  __TIC__(POSEDETECT_DPU_FC)
  configurable_dpu_task_->run(1);
  __TOC__(POSEDETECT_DPU_FC)

  auto ret = vitis::ai::pose_detect_post_process(configurable_dpu_task_->getInputTensor(), configurable_dpu_task_->getOutputTensor(), images[0].size());
  return ret;
}
