...
int main(int argc, char *argv[])
{
  ...
  for (auto batch_idx = 0u; batch_idx < results.size(); batch_idx++) {
    resize(results_show[batch_idx].segmentation, results_show[batch_idx].segmentation,
           cv::Size(results_show[batch_idx].width, results_show[batch_idx].height));
    std::cout << results_show[batch_idx].vehicle.size() << std::endl;
    for (size_t i = 0; i < results_show[batch_idx].vehicle.size(); i++) {
      int xmin = results_show[batch_idx].vehicle[i].x * results_show[batch_idx].segmentation.cols;
      int ymin = results_show[batch_idx].vehicle[i].y * results_show[batch_idx].segmentation.rows;
      int width = results_show[batch_idx].vehicle[i].width * results_show[batch_idx].segmentation.cols;
      int height = results_show[batch_idx].vehicle[i].height * results_show[batch_idx].segmentation.rows;
      cv::rectangle(results_show[batch_idx].segmentation, cv::Rect_<int>(xmin, ymin, width, height),
                cv::Scalar(185, 181, 178), 2, 1, 0);
      std::cout << "batch index: " << batch_idx                    //
           << "car: " << i                                     //
           << " score: " << results_show[batch_idx].vehicle[i].score << " " //
           << " orientation " << results_show[batch_idx].vehicle[i].angle << std::endl;
    }
    std::string file_name = get_file_name(batch_images_names[batch_idx]);
