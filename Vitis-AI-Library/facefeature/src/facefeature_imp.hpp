...
class FaceFeatureImp : public vitis::ai::TConfigurableDpuTask<FaceFeature> {
  public:
    ...
    virtual std::vector<FaceFeatureFloatResult> run(const std::vector<cv::Mat> &input_images) override;
    virtual std::vector<FaceFeatureFixedResult> run_fixed(const std::vector<cv::Mat> &input_images) override;
  private:
