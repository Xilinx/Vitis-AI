...
FaceFeatureImp::FaceFeatureImp(const std::string &model_name, bool need_preprocess)
        : vitis::ai::TConfigurableDpuTask<FaceFeature>(model_name, need_preprocess) {
}
...
std::vector<FaceFeatureFixedResult> FaceFeatureImp::run_fixed(const std::vector<cv::Mat> &input_images) {
...
std::vector<FaceFeatureFloatResult> FaceFeatureImp::run(const std::vector<cv::Mat> &input_images) {
  run_internal(input_images);
  __TIC__(FACE_FEATURE_POST_ARM)
  auto result = vitis::ai::face_feature_post_process_float(
      configurable_dpu_task_->getInputTensor(),
