...
int main(int argc,char* argv[]){
  ...
  for (auto batch_idx = 0u; batch_idx < batch; batch_idx++) {    
    ...
    cv::resize(batch_images[batch_idx], img_resize, cv::Size(width, height), 0, 0,
               cv::INTER_LINEAR);
    cv::imwrite("resize_after_" + std::to_string(batch_idx) + batch_images_names[batch_idx], img_resize);
