...
if [ $USER == "root" ]; then
  ...
  echo "export VERSION=`cat /etc/VERSION.txt`" >> $HOME/.bashrc
  ...
  echo "export LD_LIBRARY_PATH=/opt/xilinx/xrt/lib:/usr/lib:/usr/lib/x86_64-linux-gnu:/usr/local/lib:/opt/vitis_ai/conda/envs/vitis-ai-tensorflow/lib">> $HOME/.bashrc
  exec $@
else
  ...
  echo "export VERSION=`cat /etc/VERSION.txt`" >> /home/$USER/.bashrc
  ...
  echo "export LD_LIBRARY_PATH=/opt/xilinx/xrt/lib:/usr/lib:/usr/lib/x86_64-linux-gnu:/usr/local/lib:/opt/vitis_ai/conda/envs/vitis-ai-tensorflow/lib">> /home/$USER/.bashrc
