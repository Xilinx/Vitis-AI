...
def create_simple_cpu_graph(g):
    create_subgraphs_for_graph(g, SubgraphDescriptor("root", {},
                                                     [
                                                         SubgraphDescriptor("cpu1",
                                                                            {"device": "CPU"},
                                                                            [])
    ]))
...
def normal_setting_for_graph(g):
    g.get_root_subgraph().set_attr("runner", {"ref": "libvitis_ai_library-graph_runner.so.1",
    ...
    for child in g.get_root_subgraph().get_children():
        if child.has_attr("device"):
            ...
            if device == "CPU" or device == "USER":
                child.set_attr("runner", {"ref": "libvitis_ai_library-cpu_task.so.1",
...
def set_reference_input(g, tensor_name, filenames):
    ...
    op = tensor.producer
    xir_extra_ops.set_op_py_buffer_type_attr(op, "data", [read_from_cache(file) for file in filenames])
...
def add_check_point2(g, tensor_name, file_entries):
    ...
    is_cpu = subgraph.has_attr(
        'device') and subgraph.get_attr('device') == 'CPU'
    attrs = {
        "md5sum": [file['md5sum'] for file in file_entries],
        "dump_directory": os.path.join("/", "tmp", user, "error_result")
    }
    if is_cpu:
        compare_op = g.create_op("compare_" + tensor_name, "compare",
                                 attrs=attrs,
                                 input_ops={"input": [op]},
                                 subgraph=subgraph)
    else:
...
def find_tensor(g, tensor_name):
    tensors = [tensor for tensor in g.get_tensors()
               if xir_extra_ops.remove_xfix(tensor.name) == tensor_name
               ]
    assert len(tensors) == 1, "cannot find tensor: tensor_name=" + str(tensor_name) + " len=" + str(len(tensors)) + \
        " all tensors: " + \
        "\n\t".join([xir_extra_ops.remove_xfix(tensor.name)
...
def read_from_cache(file):
    ...
    md5sum = file['md5sum']
    size = file['size']
    root = os.environ.get('GOLDEN_CACHE', os.path.join(
        '/', 'scratch', 'models', 'cache', 'golden'))
