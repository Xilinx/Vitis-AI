...
static int find_sub (const std::string & output , const std::vector<std::vector<vitis::ai::library::OutputTensor>>& outputs) {
        auto ret = -1;
...
Covid19SegmentationResult Covid19SegmentationImp::run_8UC1(const cv::Mat& input_image) {
  cv::Mat image;
  ...
  __TIC__(post)
  auto sub_sc = find_sub(configurable_dpu_task_->getConfig().platenum_param().output_tensor_name()[0], configurable_dpu_task_->getOutputTensor());
  auto sub_mc = find_sub(configurable_dpu_task_->getConfig().platenum_param().output_tensor_name()[1], configurable_dpu_task_->getOutputTensor());
  ...
  return Covid19SegmentationResult{getInputWidth(), getInputHeight(), result_sc[0].segmentation, result_mc[0].segmentation};
}
...
std::vector<Covid19SegmentationResult> Covid19SegmentationImp::run_8UC1(
  ...
  for (size_t i = 0; i < seg_results.size(); i++) {
    seg_results.push_back(Covid19SegmentationResult{getInputWidth(), getInputHeight(), result_sc[i].segmentation, result_mc[i].segmentation});
  }
  __TOC__(post)
  return seg_results;
}

Covid19SegmentationResult Covid19SegmentationImp::run_8UC3(const cv::Mat& input_image) {
  cv::Mat image;
  auto size = cv::Size(getInputWidth(), getInputHeight());
  if (size != input_image.size()) {
