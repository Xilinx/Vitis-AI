...
struct MyOpImp : public vart::experimental::OpImpBase {
  MyOpImp(xir::Op* op, xir::Attrs* attrs)
    ...
    CHECK_EQ(paddings.size(), 8) << "only support 4d pad current;" << endl;
  ...
  int calculate(vart::experimental::simple_tensor_buffer_t<int8_t> output,
	...
	if(pad_.b > 0) {
	  std::fill_n(dst + (output_shape[1] - pad_.b) * dst_row_size,
        ...
        if(pad_.r > 0) {
          for(auto h=pad_.t; h<output_shape[1]-pad_.b; h++) {
            auto offset = h * dst_row_size + (output_shape[2] - pad_.r) * output_shape[3];
          ...
          for (int w = 0; w < output_shape[2]; w++) {
            for (int c = 0; c < output_shape[3]; c++) {
              ...
              if (h < pad_.t)
                ...
                h_idx = input_shape[1] - 1 - std::min(h - pad_.t - input_shape[1], input_shape[1] - 1);
              if (w < pad_.l)
                w_idx = pad_.l - 1 - std::min(w, pad_.l - 1);
              else if (w >= input_shape[2] + pad_.l)
                w_idx = input_shape[2] - 1 - std::min(w - pad_.l - input_shape[2], input_shape[2] - 1);
              auto out_idx = ((n * output_shape[1] + h) * output_shape[2] + w) * output_shape[3] + c;
              auto in_idx = ((n * input_shape[1] + h_idx) * input_shape[2] + w_idx) * input_shape[3] + c;
