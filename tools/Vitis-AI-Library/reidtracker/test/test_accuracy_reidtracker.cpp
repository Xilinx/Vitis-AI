...
void run(){
  ...
  vector<vector<vector<float>>> dets(imgCount + 1, vector<vector<float>>(0,vector<float>(7)) );
  ...
  string outfile = "MOT-" + baseImagePath.substr(baseImagePath.size() - 3, 2) + ".txt";
  ofstream of(outfile);
  while (1) {
    ...
    for(auto box : results){
      Rect2f in_box = Rect2f(box[2]/1920, box[3]/1080, box[4]/1920, box[5]/1080);
