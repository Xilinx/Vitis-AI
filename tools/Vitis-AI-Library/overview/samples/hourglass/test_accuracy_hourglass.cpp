  ...
  virtual int run() override {
    ...
    while(1){
      ...
      json_object *str_imageid = json_object_new_string(
          //dpu_result.single_name.substr(0, dpu_result.single_name.size() - 4).c_str());
      ...
      LOG_IF(INFO, 0)
      <<"test hourglass, queue size: "<<queue_->size()
      <<" dpu_result id: "<<dpu_result.frame_id; 
      if(g_last_frame_id == int(dpu_result.frame_id)){
        of << "]";
        of.close();
        LOG(INFO)
        <<"test hourglass accuracy done! byebye "<<queue_->size();
        exit(0);
      }
    }
    return 0;
...
int main(int argc, char *argv[]) {
  ...
  return vitis::ai::main_for_accuracy_demo(
      argc, argv,
      [model] {
        return vitis::ai::Hourglass::create(model);
      },
      vitis::ai::HourglassAccThread::instance(),
      2,
