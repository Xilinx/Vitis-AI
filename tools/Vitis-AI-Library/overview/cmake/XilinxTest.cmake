...
macro(XILINX_AI_SDK_ADD_ACCURACY MODEL MODEL_NAME)
  ...
  if(XILINX_AI_SDK_HAVE_${MODEL}_HEADER)
    ...
    target_link_libraries(test_accuracy_${MODEL_NAME} ${OpenCV_LIBS} ${PROJECT_NAME}::model_config ${PROJECT_NAME}::${MODEL} PkgConfig::PKG_GLOG json-c xnnpp ${PROJECT_NAME}::math ${Pthread_LIB})
...
macro(XILINX_AI_SDK_ADD_CUSTOMER_PROVIDED_MODEL_TEST TEST_CLASS)
    ...
    target_link_libraries(test_customer_provided_model_${TEST_CLASS} ${OpenCV_LIBS} dp${TEST_CLASS} PkgConfig::PKG_GLOG ${PROJECT_NAME}::dpu_task ${PROJECT_NAME}::math ${Pthread_LIB})
...
macro(XILINX_AI_SDK_TEST_BY_NAME LIB_NAME MODEL_CLASS)
  ...
  if(XILINX_AI_SDK_HAVE_${LIB_NAME}_HEADER)
    ...
    target_link_libraries(test_jpeg_${LIB_NAME} ${OpenCV_LIBS} ${PROJECT_NAME}::${LIB_NAME} PkgConfig::PKG_GLOG  ${PROJECT_NAME}::xnnpp ${PROJECT_NAME}::math ${Pthread_LIB})
...
target_include_directories(test_video_${LIB_NAME}${TMPSUFFIX} PRIVATE ${CMAKE_CURRENT_SOURCE_DIR}/samples/${LIB_NAME}/)
      target_link_libraries(test_video_${LIB_NAME}${TMPSUFFIX} ${OpenCV_LIBS} ${PROJECT_NAME}::${LIB_NAME} ${PROJECT_NAME}::xnnpp PkgConfig::PKG_GLOG ${Pthread_LIB})
      if(HAVE_DRM)
    ...
    target_link_libraries(test_performance_${LIB_NAME}${TMPSUFFIX} ${OpenCV_LIBS} ${PROJECT_NAME}::${LIB_NAME} ${PROJECT_NAME}::xnnpp vart::util PkgConfig::PKG_GLOG ${Pthread_LIB})
    ...
    file(GLOB IMGS ${CMAKE_CURRENT_SOURCE_DIR}/samples/${LIB_NAME}/*.jpg ${CMAKE_CURRENT_SOURCE_DIR}/samples/${LIB_NAME}/readme ${CMAKE_CURRENT_SOURCE_DIR}/samples/${LIB_NAME}/*.list)
    foreach(IMG ${IMGS})
      install(FILES ${IMG} DESTINATION ${SAMPLE_INATLL_PATH}/${LIB_NAME})
    endforeach()

    install(FILES  ${CMAKE_CURRENT_SOURCE_DIR}/samples/${LIB_NAME}/build.sh  
