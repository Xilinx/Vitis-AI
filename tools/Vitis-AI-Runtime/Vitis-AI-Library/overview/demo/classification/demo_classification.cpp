...
int main(int argc, char* argv[]) {
  {
    ...
    auto runner = dynamic_cast<vart::RunnerExt*>(runner_base.get());
    auto input_scale = vart::get_input_scale(runner->get_input_tensors());
    auto output_scale = vart::get_output_scale(runner->get_output_tensors());
...
static std::vector<std::vector<std::pair<int, float>>> post_process(
  ...
  for (auto i = 0; i < tensor_buffer->get_tensor()->get_shape().at(0); i++) {
...
static std::vector<float> convert_fixpoint_to_float(
  ...
  uint64_t data = 0u;
  size_t size = 0u;
  std::tie(data, size) = tensor_buffer->data({0, 0});
  auto batch = tensor_buffer->get_tensor()->get_shape().at(0);
