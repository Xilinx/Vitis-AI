...
#include "rnn_controller_u25.hpp"

#include <memory>
#include <utility>

#include "regmap_rnn_u25.hpp"
...
std::vector<uint32_t> RnnControllerU25::get_reg_data(int frame,
  ...
  if (board == "u25") {
    ...
    for (int i = 0; i < nlayers_; ++i) {
      ...
      int offset_load0 = (1 - load0_reg_dir) * (frame - 1) * actX_size + THREAD_OFFSET;
      int offset_save0 = (1 - save0_reg_dir) * (frame - 1) * actH_size + THREAD_OFFSET;
...
struct RegisterU25 {
  RegisterU25() {
    RnnControllerCreate::Register(
        "U25",
        [](unsigned int device_core_id, std::string device) {
