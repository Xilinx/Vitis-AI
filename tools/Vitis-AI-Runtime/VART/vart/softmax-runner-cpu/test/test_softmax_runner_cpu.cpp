...
static void softmax_c(const int8_t* input, float scale, unsigned int cls,
  ...
  std::vector<float> scaledIn (cls); scaledIn.push_back(max);
...
int main(int argc, char* argv[]) {
  ...
  // prepare tensors
  CHECK_EQ(s->get_input_tensors().size(), 1u);
  CHECK_EQ(s->get_output_tensors().size(), 1u);

  auto input_tensor = *(s->get_input_tensors().begin());
  auto output_tensor = *(s->get_output_tensors().begin());
  ...
  LOG_IF(INFO, ENV_PARAM(DEBUG_TEST)) 
      << "output_addr: " << (void*)output_addr << "; output_size: " << output_size;
  ...
  compare (cls, group, inputPtr, reinterpret_cast<float*>(output_addr), &output_c[0]);
