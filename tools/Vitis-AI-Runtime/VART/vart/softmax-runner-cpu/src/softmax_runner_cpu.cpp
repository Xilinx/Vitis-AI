...
void CPUCalcSoftmax(const int8_t *data, size_t size, float *result, float scale) {
  ...
  std::vector<float> input (size); input[0] = max;
...
SoftmaxRunnerCPU::SoftmaxRunnerCPU(const xir::Subgraph* subgraph, xir::Attrs* attrs)
    : input_{},
      output_{} {
  ...
  size_t batch = attrs->has_attr("__batch__") ?
                 attrs->get_attr<size_t>("__batch__") : 1;
  
  std::set<xir::Tensor*> inputTensors;
  std::set<xir::Tensor*> outputTensors;

  for (auto tensor: input_set) {
    std::vector<std::int32_t> shape = tensor->get_shape(); shape[0] = batch;
    inputTensors.emplace(xir::Tensor::create(
      tensor->get_name(), shape, 
      tensor->get_data_type()).release()
    );
  }
  for (auto tensor: output_set) {
    std::vector<std::int32_t> shape = tensor->get_shape(); shape[0] = batch;
    outputTensors.emplace(xir::Tensor::create(
      tensor->get_name(), shape, 
      tensor->get_data_type()).release()
    );
  }
    ...
    const std::vector<vart::TensorBuffer*>& output) 
{
  ...
  for (auto b = 0; b < input_batch_size; ++b)
  {
    ...
    int fixpos = input_buffer->get_tensor()->template get_attr<int>("fix_point");
    ...
    for (uint32_t i = 0; i < cls; ++i)
      out_data[i] = softmaxOut[i];
...
extern "C" vart::Runner* create_runner_with_attrs(
