...
class QuantizeWrapper(tf.keras.layers.Wrapper):
  ...
  def build(self, input_shape):
    ...
    def _get_avgpool_scaling(pool_size_h, pool_size_w):
      if pool_size_h != pool_size_w:
        return None
      elif pool_size_h == 3:
        return 9.0 * 7.0 / 64.0
      elif pool_size_h == 5:
    ...
    if isinstance(self.layer, tf.keras.layers.GlobalAveragePooling2D):
      self._avgpool_scaling = _get_avgpool_scaling(input_shape[1],
                                                   input_shape[2])
      if self._avgpool_scaling:
        print(
            'Scale output of AveragePooling2D layer({}) with factor({})for DPU simulation.'
            .format(self.layer.name, self._avgpool_scaling))

    if isinstance(self.layer, tf.keras.layers.AveragePooling2D):
      pool_size_h, pool_size_w = self.layer.get_config()['pool_size']
      self._avgpool_scaling = _get_avgpool_scaling(pool_size_h, pool_size_w)
      if self._avgpool_scaling:
        print(
            'Scale output of AveragePooling2D layer({}) with factor({})for DPU simulation.'
            .format(self.layer.name, self._avgpool_scaling))
  ...
  def call(self, inputs, training=None):
    ...
    # Handel layers with single outputs
