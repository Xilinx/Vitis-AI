...
class ClassificationImreadPreInt8 : public AKS::KernelBase
{
  public:
    int exec_async (
...
extern "C" { /// Add this to make this available for python bindings
  ...
  {
    return new ClassificationImreadPreInt8();
  }

}//extern "C"

int ClassificationImreadPreInt8::exec_async (
    std::vector<AKS::DataDescriptor*> &in, 
    std::vector<AKS::DataDescriptor*> &out, 
    AKS::NodeParams* nodeParams,
    AKS::DynamicParamValues* dynParams)
{
  ...
  AKS::DataDescriptor * outDD = new AKS::DataDescriptor(shape, AKS::DataType::INT8);
  int8_t * outData = (int8_t*) outDD->data();
  ...
  for(int i=0; i < batchSize; ++i) {
    ...
    /// Pre-Processing loop
    int8_t* batchData = outData + i * nelemsPerImg;
        ...
        for (int h = 0; h < outHeight; h++)
          for (int w = 0; w < outWidth; w++) {
            batchData[(c*outHeight*outWidth)
              + (h*outWidth) + w]
              = int8_t((resizedImage.at<cv::Vec3b>(h,w)[c]-mean[c]) / 2);
          }
