...
void runSegmentation(vitis::ai::DpuRunner *runner, bool &is_running) {
    //init out data
    float *result = new float[shapes.outTensorList[0].size];
    float *imageInputs = new float[shapes.inTensorList[0].size];
	...
	if (runner->get_tensor_format() == DpuRunner::TensorFormat::NHWC) {
            ...
            for (int c =0; c< 3; c++)
                for (int h =0; h< inHeight;h++)
                    for (int w =0; w< inWidth;w++)
                            imageInputs[c*inWidth*inHeight+h*inWidth+w] =img.at<Vec3b>(h,w)[c]-mean[c];
