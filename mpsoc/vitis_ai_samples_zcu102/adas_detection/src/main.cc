...
void displayFrame() {
    ...
    while (true) {
        ...
        if (queueShow.empty()) {
            ...
            cv::imshow("ADAS Detection@Xlinx DPU", frame);
...
void runYOLO(vitis::ai::DpuRunner* runner) {
    ...
    float *data = new float[shapes.inTensorList[0].size];
    float *result0 = new float[shapes.outTensorList[0].size];
    float *result1 = new float[shapes.outTensorList[1].size];
    float *result2 = new float[shapes.outTensorList[2].size];
    float *result3 = new float[shapes.outTensorList[3].size];
    vector<float*> result;
