...
void SSD::Init(const string& path, const string& input_node, const string& output_loc,
    ...
    getTensorShape(ssd_runner, &shapes,1, 2);
    ...
    loc = new float[loc_size];
    conf = new float[conf_size];
    loc_scale = 1.0;
...
void SSD::Run(Mat & img, MultiDetObjects * results) {
    auto inputTensors = ssd_runner->get_input_tensors();
    auto outputTensors = ssd_runner->get_output_tensors();
