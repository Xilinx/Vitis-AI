...
void GestureDetect::Init(string& path) {
    ...
    getTensorShape(fc_pt_runner, &fc_shapes,1, 2);
}
...
void GestureDetect::Run(cv::Mat& img) {
    ...
    auto inTensors = pt_runner->get_input_tensors();
    auto outTensors = pt_runner->get_output_tensors();
    ...
    float *results1 = new float[outSize];
    ...
    inputs2.push_back(vitis::ai::CpuFlatTensorBuffer(imageInputs,inTensors[0]));
    outputs2.push_back(vitis::ai::CpuFlatTensorBuffer(results1,outTensors[0]));
    ...
    inTensors = fc_pt_runner->get_input_tensors();
    outTensors = fc_pt_runner->get_output_tensors();
    
    inSize = fc_inshapes[0].size;
    outSize = fc_outshapes[0].size;
    int outSize2 = fc_outshapes[1].size;
    float* datain0 = new float[inSize];
    float* dataresult = new float[outSize];
    float* dataresult2 = new float[outSize2];
    ...
    inputs.push_back(vitis::ai::CpuFlatTensorBuffer(datain0,inTensors[0]));
    outputs.push_back(vitis::ai::CpuFlatTensorBuffer(dataresult,outTensors[0]));
    outputs.push_back(vitis::ai::CpuFlatTensorBuffer(dataresult2,outTensors[1]));
