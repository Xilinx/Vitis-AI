...
//     cv::Mat &m1, const xilinx::multitask::MultiTaskResult &result,
//     bool is_jpeg) {
...
static cv::Mat process_result_multitask_with_original(
    cv::Mat &m1, const xilinx::ai::MultiTaskResult &result,
    bool is_jpeg) {
...
static void DrawLines(Mat& img, const xilinx::ai::PoseDetectResult::Pose14Pt& results) {
  float scale_w = 1;
  float scale_h = 1;
...
process_result_pose_detect(cv::Mat &image,
               const xilinx::ai::PoseDetectResult &results,
               bool is_jpeg) {
...
static cv::Mat process_result_pose_detect_with_ssd(
    cv::Mat &image,
    const std::vector<xilinx::ai::PoseDetectResult> &results,
    bool is_jpeg) {
...
int main(int argc, char *argv[]) {
  ...
  return xilinx::ai::main_for_video_demo_multiple_channel(
      argc, argv,
      {[] {
         return xilinx::ai::create_dpu_filter(
             [] { return xilinx::ai::MultiTask8UC3::create("multi_task"); },
             process_result_multitask_with_original);
       },
       [] {
        return xilinx::ai::create_dpu_filter(
            [] { return xilinx::ai::SSDPoseDetect::create(); },
