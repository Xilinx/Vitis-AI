...
class DpuTaskImp : public DpuTask {
 ...
 public:
  ...
  virtual std::vector<xilinx::ai::InputTensor> getInputTensor(
      size_t idx) override;
  virtual std::vector<xilinx::ai::OutputTensor> getOutputTensor(
      size_t idx) override;
  virtual size_t get_num_of_tasks() const override;
