...
int main(int argc, char *argv[]) {
  ...
  auto model1 = xilinx::ai::find(argv[1]);
  model1->mutable_yolo_v3_param()->set_test_map(true);
  //  model1->mutable_yolo_v3_param()->set_conf_threshold(0.005);

  std::cout << "model1.mutable_yolo_v3_param()->test_map() "
            << model1->mutable_yolo_v3_param()->test_map() << " " //
            << "model1.mutable_yolo_v3_param()->conf_threshold() "
            << model1->mutable_yolo_v3_param()->conf_threshold() << " " //
            << "model1 " << &model1 << " "                              //
            << std::endl;
  ...
  //  auto yolo =
  //    xilinx::ai::YOLOv3::create(xilinx::ai::YOLOV3_VOC_416x416_TF, true);
