...
using namespace std;
static void DrawLines(cv::Mat& img, const xilinx::ai::PoseDetectResult::Pose14Pt& results);
...
int main(int argc, char *argv[]) {
  auto det = xilinx::ai::PoseDetect::create("ssd_pedestrain_pruned_0_97");
  vector<string> names;
  ...
  for (auto name: names) {
    ...
    vector<float> pose14pt_arry((float*)&result.pose14pt, (float*)&result.pose14pt + 28);
    for (size_t i = 0; i < pose14pt_arry.size(); i = i + 2) {
      fo << pose14pt_arry[i] * 128.f << " " << pose14pt_arry[i + 1] * 224.f << endl;
      cout << "(" << pose14pt_arry[i] * 128.f << "," << pose14pt_arry[i + 1] * 224.f << ")" << endl;
    }
