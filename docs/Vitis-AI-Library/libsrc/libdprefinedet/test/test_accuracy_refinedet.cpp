...
void parseImage(xilinx::ai::RefineDet *refinedet, cv::Mat &img,
...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    cerr << "usage: test_refinedet_accuracy file_list_file_name " << endl;
    abort();
  }

  auto refinedet =
      xilinx::ai::RefineDet::create("refinedet_pruned_0_8", true);
...
void parseImage(xilinx::ai::RefineDet *refinedet, cv::Mat &img,
  ...
  for (auto &it : results.bboxes) {
    out << single_name.substr(0, single_name.size() - 4) << " "
        << "person"
        << " " << it.score << " " << it.x * img.cols << " " << it.y * img.rows
