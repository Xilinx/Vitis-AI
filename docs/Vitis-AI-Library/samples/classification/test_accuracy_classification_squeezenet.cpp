...
int main(int argc, char* argv[]) {
  ...
  auto scale = xilinx::ai::tensor_scale(task->getOutputTensor(0u)[0]);
  ...
  for (int i = 0; i < count; i++) {
    ...
    task->run(0u);
    auto pre_data = (int8_t*)task->getOutputTensor(0u)[0].data;
    std::vector<int8_t> data(cls);
    xilinx::ai::globalAvePool(pre_data, cls, ot_w, ot_h, data.data());
