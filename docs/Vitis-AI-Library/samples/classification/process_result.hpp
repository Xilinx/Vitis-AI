...
cv::Mat process_result(cv::Mat &image,
                       const xilinx::ai::ClassificationResult &result,
                       bool is_jpeg) {
  auto x = 10;
  auto y = 20;
  auto i = 0;
  for (auto &r : result.scores) {
    ...
    LOG_IF(INFO, is_jpeg) << "r.index " << r.index << " " //
                          << xilinx::ai::Classification::lookup(r.index) << " "
                          << "r.score " << r.score << " " //
                          << std::endl;
    auto cls = std::string("") + xilinx::ai::Classification::lookup(r.index) +
               " prob. " + std::to_string(r.score);
