...
using namespace cv;
using namespace std;
using Result = xilinx::ai::OpenPoseResult::PosePoint;

static cv::Mat process_result(cv::Mat &image,
                              xilinx::ai::OpenPoseResult results,
                              bool is_jpeg) {
  vector<vector<int>> limbSeq = {{0, 1},  {1, 2},   {2, 3},  {3, 4}, {1, 5},
                                 {5, 6},  {6, 7},   {1, 8},  {8, 9}, {9, 10},
                                 {1, 11}, {11, 12}, {12, 13}};
  float scale_x = float(image.cols) / float(results.width);
  float scale_y = float(image.rows) / float(results.height);
  for (size_t k = 1; k < results.poses.size(); ++k) {
    for (size_t i = 0; i < results.poses[k].size(); ++i) {
      if (results.poses[k][i].type == 1) {
        results.poses[k][i].point.x *= scale_x;
        results.poses[k][i].point.y *= scale_y;
